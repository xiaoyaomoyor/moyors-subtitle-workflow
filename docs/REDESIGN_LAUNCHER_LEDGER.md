...
1. **绑定嵌套**：清理按钮的三个 addEventListener 被误插进 `clearCoverCache` 点击处理器的函数体内部——只有先点「清理最近工程封面缓存」才会挂上清理监听，直接点「检查失效工程记录」时监听根本不存在。
2. **桥接名不匹配**：前端调用 `registry_cleanup_preview`，后端方法名为 `preview_registry_cleanup`（apply/restore 两个本就一致）——即使监听执行，调用也会失败；且三个处理器无 try/catch，异常成未处理拒绝，界面零反馈（正是「没有任何反应」的直接表现）。

修复：绑定移出到顶层并恢复 clearCoverCache 原处理器结构；后端方法重命名 `registry_cleanup_preview` 对齐；三处理器各加 try/catch——桥接异常写入状态行（操作失败：原因），不再静默。mock 备份路径反斜杠蚀刻一并修正。

验证：修复探针 6 步全通（预览显示「发现 2 条失效记录（临时目录内且文件已缺失，共 3 项）」→ 清理/恢复按钮出现 → 确认对话框 → 清理反馈含备份路径 → 恢复反馈 3 条 → 无页面错误）；新增契约 `test_registry_cleanup_bindings_are_toplevel_and_names_align`（前后端桥接名集合对齐 + 三绑定顶层缩进 + clearCoverCache 块内无清理代码 + 失败反馈计数）；test_gui_web 256 OK + e2e 26/26 + 全量 1630 OK + ruff 全绿。

教训入账：字符串锚点插入多行处理器体内（首次插入时锚定在语句首行行尾）——后续此类插入必须锚定「完整语句块的闭合之后」并以语法检查+可点击探针即时验证；桥接名前后端集合对齐进契约防再犯。

## T5：生产路径回归与交付（三轮审查 2026-09-19 第六批·终批）

状态：已完成（2026-09-19）。依据《启动器第三轮审查》T5；基线 316a4b6。

改动与验证：home e2e 新增 2 项真实结构用例（非重叠集合+全失效目录 / DOM 顺序+主题切换保持，order/单调/序号数组断言）；r6_real 新增 T5 真实闭环 2 项（提帧→媒体名索引→分页检索命中；输出契约端到端 exportSrt=False 只产工程），全组 10/10；**原生窗口验证首次成功**（1200×780、标题正确、19px 系统标题栏+暗色内容、无浏览器壳——像素核验，三轮累计 InPrivate 壳问题未复现）；全量 1628 OK + ruff 全绿 + e2e 90/90 + 视觉矩阵 13/13 + 零写入终验 547 不变；CHANGELOG/账本/审查文档全部收口。

未验证（终批）：原生交互级操作（文件选择/拖入/右键/双会话）、真实回收站还原、跨进程注册真机并发、真实云端、便携打包、547 条清理执行（入口就绪待用户）。

## T4：设置边距、窄窗抽屉与视觉统一（三轮审查 2026-09-19 第五批）

状态：已完成（2026-09-19）。依据《启动器第三轮审查》T4（U6、A2/A3）；基线 94d84ee。

改动：settingsRailToggle 接入 tools.js 通用抽屉控制（打开定位活动分组/选中收起/Esc/遮罩/切页/resize 清理）；基础 display:none 规则（关闭头+遮罩）移到窄屏媒体查询之前（后置覆盖致窄屏头部与遮罩永不可见）；.settings-scroll 补 24px 左内边距（正文与标题同栅格，实测 88=88）；窄窗关闭态工具/设置栏 inert（键盘不可达），开合/resize 同步。

验证：探针 8/8；e2e 88/88；契约 255 OK（新增 T4：控制器接线/规则顺序索引/边距）。探针关闭判定初版 transform 恒真——改视口位置/类判定。

## T3：模块顺序、输出有效值和设置深链（三轮审查 2026-09-19 第四批）

状态：已完成（2026-09-19）。依据《启动器第三轮审查》T3（U5、A1/A8/A9）；基线 1df5f7d。

改动：波形卡移至识别卡前（DOM=注册表序，序号/视觉/键盘一致）；输出有效配置按模块开关分离（关闭时目录/主名清空+导出回默认 true，草稿保留，plan.js 与 postprocess.js 同步）；输出深链 filesPanel→settingsFilesPanel；口播对齐名称去省略号；ASR 卡移除「存入本地环境」按钮（持久化收口设置·服务与连接，新增深链）。

验证：探针 7/7；e2e 88/88；契约 254 OK（新增 T3）。过程修复：深链注释换行 shell 蚀刻致整行被注释（Edit 修正）。

## T2：真正分页、全目录搜索与计数（三轮审查 2026-09-19 第三批）

状态：已完成（2026-09-19）。依据《启动器第三轮审查》T2（U4、A6/A7）；基线 e7cccc9。

改动：后端 all_projects_payload 分页契约（page/pageSize→items/total/matched/pages/mediaIndexed，越界钳制，0 条安全，**移除 5000 硬截断**）；媒体名轻量索引 launcher-media-index.json（统计/封面写回、原子写、幂等）供全目录搜索（未看过工程可按媒体名命中，mediaIndexed 如实显示范围）；前端页码控件（上一页/省略号页码/下一页/第 X/Y 页·共 N 项，aria-current，首末禁用，0 条无分页，翻页只替换当前页）；搜索服务端化（防抖回第一页、命中/总数计数）；翻页保留选择（仅搜索彻底排除经满页成员判定才清）；请求序号防慢响应回退。

验证：探针 8/8；后端 4 项新单测（含 5200 条夹具 434 页完整访问）36 项 OK；home e2e 18/18（新增 2 项）。过程修复：mock 路径反斜杠两次蚀刻（Edit 工具修正）；搜索排除成员判定误用所选路径自查（改按用户查询词满页判定）。

未验证：命中 >120 的排除判定保守不清（宁可不误清）；存量未浏览工程首搜媒体名可能未命中（索引渐进，范围已显示）。

## T1：首页封面、图钉和失效状态（三轮审查 2026-09-19 第二批）

状态：已完成（2026-09-19）。依据《启动器第三轮审查》T1（U1/U2、A4/A5）；基线 52f7b60。

改动：图钉描边移 SVG 根（针杆 M12 17v5 继承，computed 断言）；render 统一收集两组可见卡并集一次 observeCovers（全部空时最近封面独立加载；折叠组不观察）；封面缓存按 sourceVersion（modifiedAt）失效（保存/换媒体/重新定位不复用旧图）；桥接失败/异常/解码失败 → failed 可重试占位；请求代次按路径（同路径换代替换旧响应，并发互不作废——首轮全局计数器并发缺陷由 e2e 超时暴露后修正）。

验证：探针 6/6；home e2e 16/16（新增 3 项）+ launcher 全套 86/86；契约 252 OK（新增 T1 契约）；全量 1620 OK+ruff 全绿。未验证：后端缩略图层未改动（无指控）；真实视频矩阵留 T5。
