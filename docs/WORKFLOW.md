...
配音贴片可通过「文件 → 导出音频」生成配音轨或原声混音 WAV，支持原声音轨、增益、空隙移除与后台导出。详见[音频导出](EDITOR_AUDIO_EXPORT.md)。
...
队列路径、关联、逐任务选择和已完成结果保存在本机草稿中，重新打开时重新检查文件；密钥不写入队列草稿。新队列不再创建旧版 `maw-batch-manifest.json`。供应商和模型等公共调用设置仍共用，后处理遵循[处理管线](POSTPROCESS_PIPELINE.md)。
...
- E/F 验证与原生宿主边界见 [实施记录](IMPLEMENT_LAUNCHER_EF_20260922.md)。
