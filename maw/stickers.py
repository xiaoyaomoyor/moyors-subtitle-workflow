...
from maw.env_config import apply_msw_env_aliases as _apply_msw_env_aliases, read_env


def apply_msw_env_aliases(env: dict[str, str] | None = None) -> None:
    """兼容旧导入入口；同一来源明确写入的 MSW_* 优先于 MAW_*。"""
    _apply_msw_env_aliases(env)
...
def load_env(path: Path | None = None) -> dict[str, str]:
    ...
    """
    return read_env(Path(path) if path is not None else default_env_path())
