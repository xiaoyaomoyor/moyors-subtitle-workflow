...
function maweDomContractCheck() {
  ...
  if (missing.length) {
    console.error('[MSWE][boot] editor DOM contract is incomplete', { missing });
  }
...
window.addEventListener('error', (event) => {
  ...
  window.MSWE_DEBUG_ERRORS = [...(window.MSWE_DEBUG_ERRORS || []), details];
  console.error('[MSWE][runtime] uncaught error', details);
});
window.addEventListener('unhandledrejection', (event) => {
  window.MSWE_DEBUG_ERRORS = [...(window.MSWE_DEBUG_ERRORS || []), {
    message: String(event.reason), stack: event.reason?.stack || null,
  }];
  console.error('[MSWE][runtime] unhandled rejection', event.reason);
});
if (!window.AsrEditorUtils) {
  console.error('[MSWE][boot] AsrEditorUtils is unavailable; editor scripts are incomplete or out of order');
}
...
const cueListPairLayout = document.getElementById('cue-list-pair-layout');
...
let cueListPlaybackKey = null;
let cueListPlaybackRows = [];
...
function applyCueListDisplaySettings({ preserveCueListScroll = true } = {}) {
  ...
  const projectHasStickers = [...DATA.segments, ...(getActiveExtensionTrack()?.segments || [])]
    .some(segment => segment.sticker || segment.sticker_ref);
...
function updateMultiSubtitleUi() {
  ...
  if (multiSubtitleDisplayMode) {
    multiSubtitleDisplayMode.value = enabled ? (getMultiSubtitleState().display_mode || 'both') : 'main';
    multiSubtitleDisplayMode.disabled = !enabled;
  }
  cueListPairLayout.disabled = !enabled || (getMultiSubtitleState().display_mode || 'both') !== 'both';
...
function applyAssetSearchVisibility() {
  const show = EDITOR_SETTINGS.assetLibraryShowSearch !== false;
  if (assetShowSearchToggle) assetShowSearchToggle.checked = show;
  // 整行一起隐藏（该行只有搜索、筛选与刷新按钮）。
  document.querySelector('.msw-asset-toolbar')?.toggleAttribute('hidden', !show);
}
assetShowSearchToggle?.addEventListener('change', () => {
  updateEditorSettings({ assetLibraryShowSearch: assetShowSearchToggle.checked });
...
cueListPairLayout.addEventListener('change', () => {
  commitProcessingEdits();
  updateEditorSettings({ cueListPairLayout: cueListPairLayout.value });
  applyCueListDisplaySettings();
});
...
function resolvedInterfaceColors() {
  // 旧自定义快照缺少卡片键时沿用随背景派生的 CSS 兜底。
  if (String(EDITOR_SETTINGS.themePreset || '').startsWith('custom:')) return { ...(EDITOR_SETTINGS.colors || {}) };
...
function disableSubtitlesInRemovedGaps() {
  ...
  if (!targetIndexes.length) {
    ...
    flashHint(
      window.MSWE_I18N?.translateText?.(message) || message,
...
function updateMultiSelectionClasses() {
  container.querySelectorAll('.multi-cue').forEach((element) => {
    ...
    const mainSelected = mainIndex >= 0 && selectedIdxs.has(mainIndex);
    const extensionSelected = extensionIndex >= 0 && selectedExtensionIdxs.has(extensionIndex);
    element.querySelectorAll('.multi-cue-column').forEach((column) => {
      const ownSelected = column.classList.contains('main') ? mainSelected : extensionSelected;
      column.classList.toggle('selected', ownSelected);
      column.classList.toggle('related-selected', selected && !ownSelected);
    });
  });
}
...
function renderAll({ waveform = 'overlay', preserveCueListScroll = true, cueListAnchor } = {}) {
  ...
  syncProjectTimebaseAndBindingOffsets(DATA, { preferFrames: false });
  cueListAnchor = preserveCueListScroll ? cueListAnchor || cueListScroll.mutationAnchor || captureCueListRenderAnchor() : null;
...
function renderCurrentCuePanel() {
  if (!cuePanel) return;
  const target = getCurrentCuePanelTarget();
  container.querySelectorAll('.current-target').forEach((element) => element.classList.remove('current-target'));
  if (target) {
    const selector = target.kind === 'extension'
      ? `.multi-cue-column[data-ext-idx="${target.index}"]`
      : `.multi-cue-column[data-main-idx="${target.index}"]`;
    container.querySelector(selector)?.classList.add('current-target');
  }
  ...
  if (cuePanelTarget) {
    ...
    cuePanelTarget.textContent = window.MSWE_I18N?.translateText?.(label) || label;
  ...
  if (empty) {
    ...
    cuePanelSticker.textContent = window.MSWE_I18N?.translateText?.('未选择') || '未选择';
    return;
  }
  ...
  if (seg.sticker) {
    ...
    cuePanelSticker.textContent = window.MSWE_I18N?.translateText?.('暂无表情包') || '暂无表情包';
    cuePanelSticker.title = window.MSWE_I18N?.translateText?.('点击添加表情包') || '点击添加表情包';
  }
  const segments = target.kind === 'extension' ? target.track.segments : DATA.segments;
...
function updateCueColorPresentation(el, colorBar, seg) {
  ...
  if (!colorBar.dataset.colorRefHandlerBound) {
    colorBar.addEventListener('click', (event) => {
      ...
      event.stopPropagation();
      const row = colorBar.closest('.multi-cue-column, .cue');
  ...
  if (seg.color) {
    const value = COLOR_BY_NAME[seg.color.name]?.value || seg.color.value || '#777';
...
function buildMultiCueColumn(segment, index, track, kind) {
  ...
  const colorBar = document.createElement('span');
  colorBar.className = 'color-bar';
  updateCueColorPresentation(column, colorBar, segment);
  header.prepend(colorBar);
  const sticker = document.createElement('span');
  sticker.className = 'sticker-slot';
  updateCueStickerPresentation(column, sticker, segment, index, {
    extensionTrack: kind === 'extension' ? track : null,
  });
  column.insertBefore(sticker, text);
  return column;
}
...
function buildDualCueEl(mainIndex, extensionIndex, track) {
  ...
  if (main) el.append(buildMultiCueColumn(main, mainIndex, track, 'main'));
  if (extension) el.append(buildMultiCueColumn(extension, extensionIndex, track, 'extension'));
  el.classList.toggle('single-sided', !main || !extension);
...
function renderColorSwatches() {
  ...
  COLOR_PALETTE.forEach((color) => {
    ...
    swatch.title = count
      ? window.MSWE_I18N?.translateText?.(`该颜色的字幕共 ${count} 条；点击只显示所选颜色`) || `该颜色的字幕共 ${count} 条；点击只显示所选颜色`
      : window.MSWE_I18N?.translateText?.('该颜色暂无字幕') || '该颜色暂无字幕';
...
function refreshCueColorRows() {
  const extensionTrack = getActiveExtensionTrack();
  container.querySelectorAll(':scope > .cue:not(.multi-dual-cue), .multi-cue-column').forEach((el) => {
    const colorBar = el.querySelector('.color-bar');
    if (!colorBar) return;
...
function refreshStickerAssignmentUi() {
  ...
  renderCurrentCuePanel();
  refreshSubtitlePreview();
  restoreCueListRenderAnchor(anchor);
}
  ...
  const restore = () => {
    ...
    if (cueEl?.isConnected && !cueEl.classList.contains('hidden') && Number.isFinite(anchor?.top)) {
      ...
      const targetTop = Math.max(top - containerRect.top - paddingTop,
        Math.min(owner === 'restore' || !anchor.centered ? anchor.top
          : top - containerRect.top + Math.max(0, (bottom - top - rect.height) / 2),
        bottom - containerRect.top - 1));
...
function scrollCueToCenter(cueEl, { owner = 'navigate' } = {}) {
  ...
  restoreCueListVisualAnchor(cueEl, {
    top: top - containerRect.top + Math.max(0, (bottom - top - rect.height) / 2), centered: true,
  }, owner);
}
...
function bindCueEvents(el, idx) {
  ...
  el.addEventListener('dblclick', (e) => {
    ...
    window.MSWE_ONBOARDING?.beginRealSplit(idx);
...
function updateCueListPlayback(tMs = window.MSWE?.resolve('audio-timeline')?.currentTimeMs()
  ?? player.currentTime * 1000, follow = false) {
  const track = multiSubtitleVisible() ? getActiveExtensionTrack() : null;
  const hitIndex = (segments) => {
    const index = findActiveSegmentIndex(segments, tMs, true);
  ...
  if (follow && cueListScroll.following && !cueListScroll.owner && !editingState && !extensionEditingState && document.activeElement !== cuePanelText
    ...
    if (followKey !== cueListScroll.playbackKey) {
      cueListScroll.playbackKey = followKey;
...
function updatePlaybackFrame() {
  ...
  updateCueListPlayback(tMs, true);
...
function buildTimelineOtio({ gapRemoved = false, includeStickers = false, includeSubtitleMarkers = true } = {}) {
  ...
  const audioSpecs = audioEntries.length || player?.tagName !== 'AUDIO'
    ? audioEntries.map((entry) => ({
      // 无音轨清单的旧工程走单轨兼容路径：entry 为 null 时不解构，避免崩溃。
      name: otioAudioTrackName(entry?.audioTrack ?? null, entry?.index ?? 0, audioEntries.length),
      kind: 'Audio',
      audioTrack: entry?.audioTrack ?? null,
      audioTrackIndex: entry?.index ?? 0,
    }))
...
// 浏览器文件选择器拿不到工程的真实路径，但 MSW 工程记录的媒体是绝对路径。
...
function configureServerWorkspaceLibrary() {
  ...
  if (!savedSelection && currentBuiltinWorkspaceName && presetWorkspaceHasLayout(getSavedPresetWorkspaces()[currentBuiltinWorkspaceName])) {
    // 启动时复用窗口布局，振幅决定仍属于当前工程。用户主动选择预设时照常应用预设振幅。
    const {waveformScale, waveformScaleAuto} = waveformEditor.settings;
    waveformEditor.setLayoutData(getSavedPresetWorkspaces()[currentBuiltinWorkspaceName], {render: false});
    Object.assign(waveformEditor.settings, {waveformScale, waveformScaleAuto});
...
function markProjectSaved(filename, backupName, { silent = false } = {}) {
  ...
  PROJECT_NAME = filename.replace(/\.(json|mosp|mosp-bak)$/i, '');
...
async function saveProjectToServer({ silent = false } = {}) {
  ...
  try {
    ...
    const unchanged = !inlineEditHasUncommittedText() && buildJson() === projectJson;
...
async function saveProjectAsToFile() {
  ...
  try {
    ...
    const unchanged = !inlineEditHasUncommittedText() && buildJson() === snapshot;
...
function translatedEditorText(text) {
  return window.MSWE_I18N?.translateText?.(text) || text;
}
...
function closeProjectMediaModal(clearPending = false) {
  ...
  setTimeout(() => window.MSWE_ONBOARDING?.scheduleStart(), 0);
}
...
function waveformWorkspaceForProject(project) {
  const workspace = project.workspace;
  const settings = workspace?.waveformSettings;
  if (project.msw && settings && !Object.hasOwn(settings, 'waveformScaleAuto')
      && Number.isFinite(Number(settings.waveformScale))) {
    return {...workspace, waveformSettings: {...settings, waveformScaleAuto: false}};
  }
  if (settings && !Object.hasOwn(settings, 'waveformScaleAuto')) {
    return {...workspace, waveformSettings: {...settings, waveformScaleAuto: true}};
  }
...
function applyCanonicalProject(data, filename) {
  ...
  cueListPlaybackKey = null;
  if (waveformEditor) {
    waveformEditor.setLayoutData(waveformWorkspaceForProject(DATA), { render: false });
...
async function openProjectFile(file, options = {}) {
  ...
  try {
    ...
    if (!isMawProject(data)) {
      flashHint('打开了错误的文件，请使用 MSW 生成的工程文件。', 'warning');
      return false;
    }
...
async function loadMediaFile(file, { localOnly = false, previewOnly = false, preservePrevious = false } = {}) {
  ...
  deferredReapeaksEpoch += 1;
  DATA.loudness = null;
  waveformEditor?.setLoudnessStats(null);
...
async function loadDeferredReapeaks() {
  ...
  try {
    ...
    scheduleDeferredReapeaksRetry(delay, epoch);
  }
}
...
// 清除全部空字幕块：删除所有没有文字内容的字幕（主轨 + 各副轨），可撤销。
function clearEmptyCues() {
  commitProcessingEdits();
  const emptyMain = DATA.segments
    .map((segment, index) => ({ segment, index }))
    .filter(({ segment }) => !(segment.text || '').trim())
    .map(({ index }) => index);
  const multi = getMultiSubtitleState();
  const emptyExtByTrack = [];
  for (const track of multi.tracks || []) {
    const empty = track.segments
      .map((segment, index) => ({ segment, index }))
      .filter(({ segment }) => !(segment.text || '').trim())
      .map(({ index }) => index);
    if (empty.length) emptyExtByTrack.push({ track, indices: empty });
  }
  if (!emptyMain.length && !emptyExtByTrack.length) {
    flashHint('没有空字幕块', 'default');
    return;
  }
  pushUndo(`清除 ${emptyMain.length + emptyExtByTrack.reduce((n, e) => n + e.indices.length, 0)} 条空字幕块`, { captureView: true });
  // 先解除待删除空段的绑定，保留另一侧有文字的字幕；不调用成对删除。
  removeBindingsForSegmentIds(emptyMain.map(index => DATA.segments[index].id),
...
function interfaceColorDefault(key) {
  ...
  if (key === 'card' && value && CSS.supports('color', value)) {
    // The default card color is derived with color-mix; color inputs need hex.
...
function applyAsrJob(job, media) {
  ...
  normalizeMultiSubtitleState();
  projectImportDirty = true;
  clearSelection({ silent: true }); cueListPlaybackKey = null;
  renderAll({ waveform: 'full' });
...
function applyTranslationJob(job) {
  ...
  if (plan.appliedIds.length) {
    ...
    if (plan.segments) DATA.segments.splice(0, DATA.segments.length, ...plan.segments);
    if (plan.multi) DATA.multi_subtitle = plan.multi;
    if (plan.multi && extension.asr_stale_subtitles) {
    ...
    if (applied.length === job.snapshot.entries.length) {
      ...
      if (!job.snapshot.track_id && plan.multi?.tracks[0]) extension.translation_target_tracks = {
...
window.MSWE?.register('persistence-host', () => Object.freeze({
  ...
  commitEdits: flushInlineEditsForSave,
  ...
  adopt: (result, { source, newProject }) => {
    ...
    projectCheckpointed = true;
    PROJECT_NAME = result.filename.replace(/\.(json|mosp|mosp-bak)$/i, '');
    FILENAME_BASE = PROJECT_NAME;
...
window.MSWE?.register('processing-host', () => Object.freeze({
  ...
  applyWaveform: (payload, layers = {}) => {
    ...
    DATA.loudness = layers.loudness || null;
    waveformEditor.setLoudnessStats(DATA.loudness);
  ...
  clearWaveform: () => {
    deferredReapeaksEpoch += 1;
    DATA.loudness = null; waveformEditor?.setLoudnessStats(null);
  ...
  mediaSnapshot: () => ({ media: DATA.media, metadata: window.MSWProject.clone(DATA.media_metadata),
    waveform: DATA.waveform, spectral: DATA.spectral, reapeaks: DATA.waveform_reapeaks, loudness: DATA.loudness,
  ...
  restoreMedia: async state => {
    ...
    DATA.loudness = state.loudness || null; waveformEditor?.setLoudnessStats(DATA.loudness);
  },
  ...
  refreshAudioPlayback: (timeMs) => {
    if (Number.isFinite(timeMs)) {
      const index = findActive(timeMs);
      nowEl.textContent = fmtShort(timeMs); updateCueListPlayback(timeMs, true); refreshSubtitlePreview(timeMs, index);
    }
    waveformEditor?.updatePlayback();
    syncMediaControls();
  },
  ...
  commitAudio: (label, change) => {
    ...
    change(next);
    window.MSWProject.normalize(next);
    try { window.MSWAudio.assertCapacity(next); }
...
window.MSWE?.register('editor-bridge', () => window.MSWE_EDITOR_BRIDGE);
renderAll({ waveform: 'full' });
