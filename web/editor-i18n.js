(function initMaweI18n(global) {
  ...
  const EN_TEXT = {
    '叠加字幕支持编辑和导出；翻译、配音暂不处理叠加轨。': 'Overlay subtitles support editing and export; translation and TTS do not target this track yet.',
    '本工程的 ASS 导出和视频烧录使用样式库': 'Use the style library for this project’s ASS and video exports',
    '默认关闭，保留原工程的导出和烧录样式。开启后使用当前样式库方案；导出开始时固定样式快照。': 'Off by default to preserve existing export styles. Enable to use the current library profile; styles are frozen when export starts.',
    'SRT 烧录': 'SRT burning',
    '计算设备': 'Compute device',
    '不使用': 'None',
    '本工程使用 ASS 样式库导出和烧录': 'Use the ASS library for this project’s exports and burning',
    '编辑 ASS 样式库': 'Edit ASS style library',
    '默认保留工程原有样式。启用后使用样式库中的主／副字幕方案，叠加字幕显示在其上方；开始导出时固定本次样式。': 'Preserve existing project styles by default. Enable the main and secondary library styles, with overlay text above them. Each export freezes its styles.',
    '读取本机字体，填充字体下拉列表': 'Read local fonts to populate the font list',
    '次色仅在 \\\\k 卡拉 OK 标签中使用，当前未支持': 'Secondary color is reserved for karaoke tags, which are not currently supported',
    '本地模型（Beta）': 'Local models (Beta)',
    '本地模型目录（可选）': 'Local model directory (optional)',
    'FireRed 标点': 'FireRed punctuation',
    'ct-punc（需下载）': 'ct-punc (download required)',
    '识别后补齐字词时间码': 'Fill word timestamps after ASR',
    '不额外对齐': 'No additional alignment',
    '对齐模型目录（可选）': 'Alignment model directory (optional)',
    '本地识别不需要 API Key。仅在点击下方按钮后下载安装，可能需要数 GB 空间；FireRed 的 ct-punc 单独按需下载。': 'Local recognition requires no API key. Downloads start only when you click below and may need several GB. FireRed ct-punc is downloaded separately on demand.',
    '安装／修复运行环境': 'Install / repair runtime',
    '下载识别模型': 'Download ASR model',
    '下载所选对齐模型': 'Download selected aligner',
    '下载 ct-punc': 'Download ct-punc',
    '取消下载': 'Cancel download',
    '刷新状态': 'Refresh status',
    '本地识别不需要 API Key，请在 ASR 面板准备模型': 'No API key is needed; prepare local models in the ASR panel.',
    '请先准备所选本地模型和运行环境，然后刷新状态': 'Prepare the selected model and runtime, then refresh status.',
...
'启用多重字幕时使用的波形行高度': 'Waveform row height used when multiple subtitles are enabled',
'请拖入第二个 srt 字幕以开启多重字幕功能': 'Drop a second SRT subtitle to enable multiple subtitles',
'请拖入第二条字幕以开启多重字幕编辑': 'Drop a second subtitle to enable multi-subtitle editing',
...
'多重字幕列表显示方式': 'Multiple-subtitle list display mode',
'作为多重字幕': 'Add as multiple subtitles',
...
'禁用波形': 'Disable waveform',
'请选择至少两个字幕块！': 'Select at least two subtitle blocks!',
    ...
    '新增音频素材': 'New audio assets',
    '新结果已存入素材库': 'New result saved to the asset library',
    '请先完成正在确认的生成任务': 'Finish confirming the pending generation first',
    '重新生成音频素材': 'Regenerate audio asset',
    '覆盖主字幕': 'Replace main subtitles',
    '存入素材库': 'Save to asset library',
    '已存入素材库': 'Saved to asset library',
    '本批完成结果已入库': 'Completed results in this batch are in the library',
    '仅预览前 300 条，入库保留全部': 'Previewing the first 300; all subtitles are saved to the library',
    '同批识别范围重叠，请先存入素材库': 'Recognition ranges in this batch overlap; save to the library first',
    '素材类型': 'Asset type',
    '字幕素材内容': 'Subtitle asset text',
    '字幕素材 · 独立编辑': 'Subtitle asset · Independent editing',
    '编辑字幕素材': 'Edit subtitle asset',
    '完成编辑': 'Finish editing',
    '复制到素材库': 'Copy to library',
    '显示底部试听栏': 'Show preview player',
    '条素材': 'assets',
    '主字幕素材': 'Primary subtitle asset',
    '副字幕素材': 'Secondary subtitle asset',
    '放入所选字幕': 'Insert selected subtitles',
    '复制字幕': 'Copied subtitles',
    '字幕素材随工程保存。': 'Subtitle assets are saved with the project.',
    '素材列表': 'Asset list',
    '选择目标副字幕轨': 'Choose a secondary subtitle track',
    '搜索文字、音色、文件名…': 'Search text, voice, filename…',
    '移动工程时请保留同目录的 .assets 文件夹；未保存工程可导出工程与音频素材包。': 'Keep the adjacent .assets folder when moving the project; export a project and audio asset bundle for unsaved projects.',
    '导出工程与音频素材包': 'Export project and audio asset bundle',
    '已导出工程与音频素材包；解压后打开 project.mosp。': 'Project and audio assets exported; unzip and open project.mosp.',
    '更多悬浮显示': 'More hover details',
    '试听音量': 'Preview gain',
    '悬浮时显示起止时间与总用时；字幕显示序号，音频贴片显示贴片与配音轨叠加后的试听增益': 'Show start, end and duration on hover; subtitle numbers and the combined clip and voice-track preview gain',
    '用上次草稿替换当前配音草稿？': 'Replace the current draft with the last cleared draft?',
    '配音片段超过 600 字符，请调整断句设置': 'A speech segment exceeds 600 characters; adjust splitting settings',
    "媒体 · 处理": "Media · Processing",
    "音频批量操作": "Batch audio actions",
    "源音频音量增益": "Source audio preview gain",
    "当前试听增益": "Current preview gain",
    "已调整源音频试听增益": "Source audio preview gain updated",
    "源音频增益须在 −60 至 +12 dB": "Source gain must be between -60 and +12 dB",
    "媒体已改变，请重新调整试听增益": "Media changed; adjust preview gain again",
    "点击播放以启用源音频试听": "Press play to enable source audio preview",
    "补齐": "Fill",
    "正在重试": "Retrying",
    "原媒体音画尾差": "Native audio/video tail difference",
    "默认按画面结尾；定格延长可保留原声尾部": "Defaults to picture end; freeze to retain the original audio tail",
    "关闭音频批量操作": "Close batch audio actions",
    "静音贴片": "Mute clips",
    "选中贴片": "Selected clips",
    "可重新生成": "Regenerable",
    "可补齐字幕": "With subtitle text",
    "调整方式": "Adjustment",
    "设置为": "Set to",
    "增减": "Offset",
    "增益（dB）": "Gain (dB)",
    "应用增益": "Apply gain",
    "重新生成并替换": "Regenerate and replace",
    "取消本批": "Cancel batch",
    "补齐字幕": "Fill subtitles",
    "补齐主字幕": "Fill primary subtitles",
    "处理详情": "Action details",
    "已处理": "Processed",
    "条贴片": "clips",
    "已补齐": "Filled",
    "未替换": "Not replaced",
    "已替换": "Replaced",
    "继续确认本批任务": "Resume batch confirmation",
    "正在重新生成": "Regenerating",
    "可继续确认本批任务，不会重复提交": "Resume this batch to confirm its status without duplicate submissions",
    "没有可重新生成的贴片": "No clips have a reusable synthesis recipe",
    "请先选择音频贴片": "Select audio clips first",
    "没有可用的文字或文件名": "No text or filename available",
    "文字超过 600 字符，请先编辑文字": "Text exceeds 600 characters; edit it first",
    "已生成文件名占位字幕，请填写真实文字": "Filename placeholder created; enter the actual subtitle text",
    "已补齐；音频已裁剪，请核对文字": "Subtitle added; verify the text for this trimmed clip",
    "与现有主字幕重叠": "Overlaps an existing primary subtitle",
    "所选贴片彼此重叠": "Selected clips overlap each other",
    "缺少原合成配置或属于外部音频": "Missing original synthesis settings or imported audio",
    "原合成文字缺失或超过 600 字符": "Original text is missing or exceeds 600 characters",
    "目标贴片已变化，新结果保留在素材库": "Clip was edited; new audio retained in the library",
    "新素材已移除，保留原贴片": "New asset was removed; original clip retained",
    "超过三层重叠，新结果保留在素材库": "Would exceed three layers; new audio retained in the library",
    "已取消替换，新结果保留在素材库": "Replacement cancelled; new audio retained in the library",
    "未替换，新结果保留在素材库": "Not replaced; new audio retained in the library",
    "合成失败或已取消": "Synthesis failed or cancelled",
    "请输入有效的增益数值": "Enter a valid gain value",
    "批量修改贴片音量增益": "Batch clip gain",
    "重新生成并替换音频贴片": "Regenerate and replace audio clips",
    "配音草稿": "Speech draft",
    "合成成功后清空配音草稿": "Clear draft after successful synthesis",
    "手动换行分段": "Split at hard line breaks",
    "自动断句": "Automatic splitting",
    "按标点": "By punctuation",
    "按字数": "By length",
    "标点与字数": "Punctuation and length",
    "断句标点": "Split punctuation",
    "每段字数上限": "Characters per segment",
    "恢复上次草稿": "Restore last draft",
    "分段预览": "Segment preview",
    "最长": "Longest",
    "段": "segments",
    "仅预览前 100 段": "Only the first 100 segments are previewed",
    "导出范围超过原视频时": "When export extends beyond the source video",
    "超出时先选择处理方式": "Require a choice when exceeded",
    "按视频结尾截断成品": "End output at the video end",
    "定格最后一帧并延长": "Extend with a frozen final frame",
    "原画面时长": "Source video duration",
    "源范围终点": "Source range end",
    "超出": "Exceeded by",
    "比较的是整个导出范围，包括尾部字幕与配音。截断会舍去成品中超出的部分；定格会延长最后一帧。仅影响本次导出，不删除工程内容。": "This compares the full export range, including trailing subtitles and audio. Truncation omits the excess from the output; freezing extends the last frame. Project content is preserved.",
    ...
    '设置 → 保存：创建或恢复磁盘版本；恢复后请另存为，原视频需单独保留。': 'Settings → Save: create or restore disk versions. Save a recovered copy as a new project; retain the source video separately.',
    '设置 → 说话人：五色映射默认关闭；预览与 SRT 名称独立开启，不修改字幕正文或配音。': 'Settings → Speakers: color mapping is off by default. Enable names separately for preview and SRT; subtitle text and synthesis stay unchanged.',
    '情感描述文本（实验）': 'Emotion description (experimental)',
    '配音语言': 'Synthesis language',
    '日本語': 'Japanese',
    '选择官方示例音色': 'Choose an official sample voice',
    '请选择或上传参考音频': 'Choose or upload reference audio',
    '官方情感示例': 'Official emotion samples',
    '选择情感示例': 'Choose an emotion sample',
    '请选择或上传情感参考': 'Choose or upload an emotion reference',
    "输入后按 Enter 或点击其他位置生效；可撤销": "Press Enter or click elsewhere to apply; supports undo",
    "正数向右、负数向左；应用后归零": "Positive moves right, negative moves left; resets after applying",
    "未选中字幕时对全部生效 · Enter 确定 · Esc 关闭": "Applies to all when nothing is selected · Enter to apply · Esc to close",
    "确认使用此源音轨": "Confirm source audio track",
    "Moyor's Subtitle Workflow Editor · 我的字幕流": "Moyor's Subtitle Workflow Editor",
    "+ 鼠标左键拖动：选择连续时间范围（Mac 可用 Cmd）；Esc 清除，右键选区精确调整": "+ left drag: select a time range (Cmd on Mac); Esc clears; right-click to adjust precisely",
    "将在当前地域、密钥和目标模型下登记，之后可在合成面板搜索选择。本机名称不改变云端音色 ID。": "Creates a voice using the current region, key and target model. Find it in synthesis settings afterward. The local name does not change its cloud ID.",
    "音色名称": "Voice name",
    "音色描述": "Voice description",
    "试听文本": "Preview text",
    "你好，欢迎使用字幕配音。Hello, welcome to our story.": "Hello, welcome to our story.",
    "参考音频": "Reference audio",
    "推荐 10–20 秒，范围 3–60 秒、最大 10 MiB。使用清晰单人说话、无背景音乐的音频；本机会转换为 24 kHz 单声道 WAV，原文件不变。": "Recommended: 10–20 seconds; allowed: 3–60 seconds, up to 10 MiB. Use clear speech from one speaker without music. A local 24 kHz mono WAV copy is prepared; the original is preserved.",
    "点击创建后，参考音频将上传至所选地域的百炼；参考文件不写入工程或本机音色记录。": "Creating uploads the reference to Bailian in the selected region. The reference is not stored in the project or local voice record.",
    "创建音色可能计费，完成后可重复用于多条字幕。状态不明时先在百炼核对，不要重复创建。": "Voice creation may incur a charge. Reuse the voice for multiple subtitles. If its status is unknown, check Bailian before creating it again.",
    "尚未检测本机资源": "Local resources not checked yet",
    "安装资源包": "Install resources",
    "取消安装／检测": "Cancel installation/check",
    "资源目录": "Resource directory",
    "应用目录并检测": "Apply directory and check",
    "自动下载独立运行时和八音色，安装后可离线使用。离线电脑也可复制此资源目录，再应用检测。": "Downloads the runtime and eight voices for offline use. On an offline computer, copy this resource directory, then apply and check it.",
    "资源来源：": "Resource sources:",
    "英文规则": "English rules",
    "。使用 AquesTalk，版权归株式会社アクエスト；保留随包许可，参阅": ". Uses AquesTalk, copyright AQUEST Corporation. Keep the bundled license; see",
    "官方许可说明": "Official license information",
    "IndexTTS 连接设置": "IndexTTS connection settings",
    "本机服务地址": "Local service URL",
    "单次等待上限（秒）": "Request timeout (seconds)",
    "连接／检测": "Connect/check",
    "使用已启动的官方 IndexTTS 2.5 WebUI，无需 API Key。计算设备和模型由该服务管理。": "Connects to a running official IndexTTS 2.5 WebUI. No API key required. The service manages its models and compute device.",
    "请先连接并检测本机服务。": "Connect and check the local service first.",
    "文本情感需 IndexTTS 加载 QwenEmotion。可用 --qwen_emo 启动该服务后重新检测；MSWE 不会自动重启服务。": "Text emotion requires QwenEmotion in IndexTTS. Start the service with --qwen_emo and check again. MSWE does not restart it automatically.",
    "上传参考": "Upload reference",
    "上传情感参考": "Upload emotion reference",
    "参考音频用于克隆音色。推荐清晰单人说话；音色和情感参考均取前 15 秒，最大 32 MiB。文件保存在本机参考库。": "Reference audio is used for voice cloning. Use clear speech from one speaker. Voice and emotion references use the first 15 seconds, up to 32 MiB. Files are stored in the local reference library.",
    "上传后选作当前音色或情感参考，可在合成设置中切换。": "Select an uploaded reference for the voice or emotion; switch it in synthesis settings.",
    "高级生成参数": "Advanced generation settings",
    "随机采样 · do_sample": "Random sampling · do_sample",
    "Top K（0 表示不限）": "Top K (0 for unlimited)",
    "温度": "Temperature",
    "长度惩罚": "Length penalty",
    "束搜索数": "Beam count",
    "重复惩罚": "Repetition penalty",
    "最大音频 Token 数": "Maximum audio tokens",
    "每段最大文本 Token 数": "Maximum text tokens per segment",
    "较低的最大音频 Token 数可能截断长句；情感向量由 IndexTTS 归一化。时长系数影响语速，不保证精确匹配字幕时长。": "Low audio token limits may truncate long sentences. IndexTTS normalizes emotion vectors. Duration scaling changes speech speed without guaranteeing an exact subtitle duration.",
    "MSWE 本机预设": "MSWE local presets",
    "预设名称": "Preset name",
    "保存为本机预设": "Save local preset",
    "IndexTTS 服务预设": "IndexTTS service presets",
    "选择服务预设": "Choose service preset",
    "载入服务预设": "Load service preset",
    "载入预设会应用音色和生成参数。服务的旧预设不包含语言与时长系数，载入时保留这两项；MSWE 本机预设保存完整参数。": "Loading applies voice and generation settings. Older service presets omit language and duration scaling, so those values are retained. MSWE local presets store all settings.",
    "选择音频试听": "Choose audio to preview",
    "跳转": "Jump",
    "倒放/停止/播放": "Reverse/Stop/Play",
    "Shift+拖拽": "Shift+drag",
    "绑定 ·": "Bind ·",
    "qwen-audio-3.0-asr（热词 / 上下文）": "qwen-audio-3.0-asr (hotwords/context)",
    "fun-asr（支持说话人）": "fun-asr (speaker support)",
    "qwen3-asr（准确率更高）": "qwen3-asr (higher accuracy)",
    "自动识别": "Auto-detect",
    "自定义模型": "Custom model",
    "短句合并阈值": "Short sentence merge threshold",
    "英文最大单词数": "Maximum English words",
    "英文短句合并阈值": "Short English sentence merge threshold",
    "停顿切句阈值（毫秒）": "Pause split threshold (ms)",
    "Qwen 上下文（最多 400 字）": "Qwen context (up to 400 characters)",
    "即时热词": "Hotwords",
    "热词表 ID": "Vocabulary ID",
    "Soniox 通用上下文": "Soniox general context",
    "Soniox 文本上下文": "Soniox text context",
    "Soniox 术语": "Soniox terms",
    "Soniox 翻译术语": "Soniox translation terms",
    "语言读法": "Pronunciation language",
    "读音修正（可选，仅此次配音）": "Pronunciation override (optional, this synthesis only)",
    "支持中英文混合，保留油库里的假名近似读法。英文数字逐位读；自动模式在含汉字时使用中文数字读法。无需 API Key。": "Supports mixed Chinese and English with Yukkuri kana approximations. English digits are read individually; auto mode uses Chinese number readings when Chinese characters are present. No API key required.",
    "语速 ·": "Speed ·",
    "情感控制": "Emotion control",
    "跟随音色参考": "Follow voice reference",
    "情感参考音频": "Emotion reference audio",
    "八维情感向量": "Eight-dimensional emotion vector",
    "情感描述文本": "Emotion description",
    "情感权重 ·": "Emotion weight ·",
    "情感随机采样（每次生成可能不同）": "Sample emotion randomly (results may vary)",
    "试听音色参考": "Preview voice reference",
    "试听情感参考": "Preview emotion reference",
    "喜 ·": "Happy ·",
    "怒 ·": "Angry ·",
    "哀 ·": "Sad ·",
    "惧 ·": "Afraid ·",
    ...
    "惊喜 ·": "Surprised ·",
    "平静 ·": "Calm ·",
    "字数阈值": "Character count threshold",
    "加载媒体或字幕文件": "Load media or subtitle file",
    "导出主/副字幕（SRT/ASS）或按颜色分别导出": "Export main/extension subtitles (SRT/ASS), or export by color",
    "导出主字幕轨": "Export main subtitle track",
    "打开缩放/偏移字幕窗口": "Open subtitle scale/shift window",
    "识别整个源视频或时间选区": "Transcribe the source video or selected time ranges",
    "播放时跳过已移除的空隙；左键定位到空隙内时可临时预览": "Skip removed gaps during playback; left-click inside a gap to preview it temporarily",
    "删除当前所有已移除和已恢复的空隙区段记录（可撤销）": "Delete all removed and restored gap records (undoable)",
    "在「字幕编辑器」等子工作区顶部栏显示 Enter / Esc 等快捷键提示（默认关闭）": "Show Enter/Esc and other shortcut hints in panel headers (off by default)",
    "Alt+左键点击切换整段，空白处 Alt+左键拖动增加空隙；空隙块左键或 Alt+左键拖动整体偏移，Ctrl/Cmd+拖动复制；中键拖动默认增加静音，按住 Alt 才恢复声音": "Alt+left click toggles a gap; Alt+left drag on blank space adds one. Left/Alt+left drag moves gaps; Ctrl/Cmd+drag copies. Middle drag adds silence; Alt restores sound.",
    "关闭缩放/偏移字幕": "Close subtitle scale/shift",
    "开启后，字数过滤期间拆分出的字幕会暂时保留，直到点击其他字幕、波形或空白处": "Keep newly split subtitles visible during count filtering until another subtitle, waveform or blank area is clicked",
    "波形形状来源：默认使用媒体旁 .ReaPeaks 的最细 wave 层（缺数据时自动回退自研缓存）；需要时可切回自研": "Waveform source: use the finest cached wave layer, falling back to native peaks when unavailable; switch to native peaks if needed",
    "在波形条左侧显示行编号（V1/V2 字幕行、A1/A2 配音行）；点击编号可禁用/启用整行": "Show lane labels at the left (V1/V2 subtitles, A1/A2 audio); click to mute/unmute a lane",
    "用于搜索和识别音色": "Used to find and identify the voice",
    "例如：温柔旁白": "For example: gentle narrator",
    "例如：年轻女声，温暖明亮，普通话清晰，语速舒缓。支持中文或英文描述。": "For example: a young, warm, bright female voice with clear Mandarin and a relaxed pace. Chinese and English descriptions are supported.",
    "油库里资源安装进度": "Yukkuri installation progress",
    "离线包解压后包含 msw-runtime.json 的目录": "Extracted offline package directory containing msw-runtime.json",
    "例如：中文旁白": "For example: Chinese narrator",
    "界面最底层背景": "Base interface background",
    "菜单栏色": "Menu bar color",
    "模块顶部标签栏（外面板）": "Panel tab bar",
    "文本输入框底色": "Text input background",
    "输入框色": "Input color",
    "主要文字": "Primary text",
    "次要说明文字": "Secondary text",
    "弱文字色": "Muted text color",
    "按钮、选中与高亮": "Buttons, selections and highlights",
    "波形峰值颜色": "Waveform peak color",
    "字幕列表/编辑区/波形块的文字色（不含播放器画面内预览）": "Text color in the subtitle list, editor and waveform blocks (excluding player preview)",
    "字幕块色": "Subtitle block color",
    "媒体模块栏": "Media panel header",
    "媒体播放器快捷键": "Media player shortcuts",
    "波形显示器快捷键": "Waveform shortcuts",
    "V1 主字幕轨——点击禁用整条轨道": "V1 main subtitles — click to mute the track",
    "字幕列表快捷键": "Subtitle list shortcuts",
    "字幕列表模块栏": "Subtitle list header",
    "可填同音字、英文或假名，不改字幕原文": "Use homophones, English or kana without changing subtitle text",
    "例如：委屈、兴奋、温柔；留空时使用每条字幕文本判断情感": "For example: hurt, excited, gentle; leave blank to infer emotion from each subtitle",
    "可用 <行|HANG2>、英文音素或日语假名，不改字幕原文": "Use a character with its pronunciation override, English phonemes or Japanese kana without changing subtitle text",
    '服务': 'Service',
    '查看百炼': 'See Bailian',
    '音色列表': 'Voice list',
    '音色创建记录': 'Voice creation history',
    '阿里云百炼（QwenASR / FunASR）': 'Alibaba Bailian (QwenASR / FunASR)',
    '豆包语音识别（火山引擎）': 'Doubao ASR (Volcengine)',
    'OpenAI（及兼容接口）': 'OpenAI (and compatible endpoints)',
    '北京（华北 2，默认）': 'Beijing (North China 2, default)',
    '新加坡（需要 Workspace ID）': 'Singapore (Workspace ID required)',
    '请先在连接设置填写或保存百炼密钥。': 'Enter or save a Bailian key in connection settings first.',
    '查看百炼 音色列表、Voice design、Voice cloning。': 'View Bailian voice list, Voice design and Voice cloning.',
    "字幕颜色与说话人": "Subtitle colors and speakers",
    "启用五色说话人映射": "Map five colors to speaker names",
    "在预览中显示说话人": "Show speaker names in preview",
    "分隔符": "Separator",
    "黄色": "Yellow",
    "绿色": "Green",
    "紫色": "Purple",
    "蓝色": "Blue",
    "预览字幕颜色样式": "Preview subtitle color style",
    "下划线": "Underline",
    "文字": "Text",
    "阴影": "Shadow",
    "描边": "Stroke",
    "名称仅用于显示和明确启用的 SRT 导出，不修改正文、原始 speaker 或 TTS 音色。": "Names apply to preview and enabled SRT exports. Subtitle text, ASR speaker IDs and TTS voices are preserved.",
    "SRT 导出说话人": "Include speaker names in SRT exports",
    "以说话人名称作为分色 SRT 后缀": "Use speaker names for color SRT filenames",
    "磁盘版本备份": "Disk version backups",
    "启用磁盘版本备份": "Enable disk version backups",
    "备份间隔（秒）": "Backup interval (seconds)",
    "最多保留版本": "Maximum versions",
    "保存备份设置": "Save backup settings",
    "立即创建版本": "Create version now",
    "打开备份目录": "Open backup folder",
    "恢复磁盘版本": "Restore disk version",
    "版本共用备份目录中的音频素材，不包含原视频；移动工程时请保留媒体和备份目录。未命名工程仍使用本机恢复草稿。": "Versions share audio assets in the backup folder and reference the original video. Keep media and backups when moving the project. Untitled projects still use local recovery drafts.",
    "主轨": "Main track",
    "副轨": "Extension track",
    "任一轨": "Either track",
    "字数过滤目标": "Character count filter track",
    "每行一个": "One per line",
    "diarized_json（说话人段落）": "diarized_json (speaker segments)",
    "接口必须返回可靠时间戳；Keywords 每行一个": "The endpoint must return reliable timestamps. One keyword per line.",
    "此模型在当前接口没有可靠字幕时间戳，请更换模型": "This model has no reliable subtitle timestamps on this endpoint. Choose another model.",
    "内容未变化，已保留现有版本": "No changes; existing version retained",
    "磁盘版本已创建": "Disk version created",
    "磁盘版本备份失败": "Disk version backup failed",
    ...
    '识别记录': 'Transcription history',
    '选择结果': 'Select result',
    '当前结果': 'Selected result',
    '候选字幕': 'Candidate subtitles',
    '识别已开始': 'Transcription started',
    'ASR 环境配置已保存': 'ASR environment settings saved',
    '媒体分析': 'Media analysis',
    '媒体工具': 'Media tools',
    '媒体分析需要本机编辑器服务。': 'Media analysis requires the local editor service.',
    '导入媒体后可生成波形或播放代理。': 'Import media to generate peaks or a playback proxy.',
    '播放代理': 'Playback proxy',
    '原媒体无法预览时生成兼容副本；识别仍使用原媒体。': 'Generate a compatible copy when the original cannot play; transcription still uses the original.',
    '前往媒体工具环境配置': 'Open media tools environment settings',
    'FFmpeg / FFprobe 用于媒体信息读取、波形、播放代理及音视频处理。': 'FFmpeg / FFprobe handle media metadata, peaks, playback proxies and audio/video processing.',
    '加选播放头至鼠标位置': 'Add range from playhead to pointer',
    '选区编辑': 'Edit range',
    '编辑片段': 'Edit interval',
    '关闭 ASR': 'Close ASR',
    '打开 ASR 面板': 'Open ASR panel',
    '前往 ASR 环境配置': 'Open ASR environment settings',
    '自动识别语言': 'Detect language automatically',
    '使用已保存的 ASR 配置': 'Using saved ASR settings',
    '请先在环境配置中保存此服务的 API Key': 'Save this service API key in environment settings first',
    '请在环境配置中配置 ASR 服务': 'Configure an ASR service in environment settings',
    '配置有未保存的修改；识别仍使用上次保存的配置': 'Unsaved changes; transcription still uses the last saved settings',
    'ASR 配置与启动器共用，保存后用于编辑器识别。密钥仅保存在本机。': 'ASR settings are shared with Launcher. Save them for editor transcription. Keys stay on this computer.',
    '多个选区会分别创建任务；结果按片段预览和应用，中间未选中的内容保持原样。': 'Each selected interval creates a separate job. Preview and apply each result; unselected gaps stay unchanged.',
    '选区编辑：拖动选择，Shift 加选，Ctrl/Cmd 减选，拖动边界调整': 'Edit range: drag to select, Shift to add, Ctrl/Cmd to subtract; drag boundaries to resize',
    '选区编辑：拖动选择波形；Shift 加选，Ctrl(Cmd) 减选；拖动选区边界扩大或缩小。Ctrl(Cmd)+Shift 拖动仍可临时选择': 'Edit range: drag to select, Shift to add, Ctrl/Cmd to subtract; drag boundaries to resize. Ctrl/Cmd+Shift drag still selects temporarily',
    '识别完成，等待结果应用': 'Transcription finished; ready to apply',
    '正在提取所选源音轨': 'Extracting the selected source audio track',
    '正在识别源音频': 'Transcribing source audio',
    '正在等待识别结果': 'Waiting for transcription results',
    '媒体正在导入，请等待完成后再识别': 'Wait for media import to finish before transcribing',
    '全部': 'All',
    '查看百炼 音色列表、Voice design、Voice cloning。': 'View the Bailian voice list, Voice design and Voice cloning.',
    '请确认本机编辑器服务仍在运行；当前无法自动保存工程，请使用「文件 → 另存为工程」保存当前工程，避免进度丢失。': 'Check that the local editor service is running. Auto-save is unavailable; use File → Save project as to preserve your edits.',
    'ASR · 源音频识别': 'ASR · Source audio transcription',
    '识别范围': 'Transcription range',
    '整个源视频': 'Entire source media',
    '扩展到完整字幕边界': 'Expand to complete subtitle boundaries',
    'ASR 服务配置': 'ASR service settings',
    '保存服务配置': 'Save service settings',
    '地域': 'Region',
    '留空自动识别': 'Leave blank to detect automatically',
    '留空使用本机已保存密钥': 'Leave blank to use the locally saved key',
    '已配置本机密钥，留空继续使用': 'A local key is configured; leave blank to keep using it',
    '尚未配置此服务密钥': 'No key configured for this service',
    '区分说话人': 'Identify speakers',
    '即时热词（每行一个）': 'Hotwords (one per line)',
    '断句、热词与上下文': 'Segmentation, hotwords and context',
    '最大字数': 'Maximum characters',
    '最小字数': 'Minimum characters',
    '最大词数': 'Maximum words',
    '最小词数': 'Minimum words',
    '停顿阈值（秒）': 'Pause threshold (seconds)',
    '音频上下文': 'Audio context',
    '热词': 'Hotwords',
    '词表 ID': 'Vocabulary ID',
    '热词权重': 'Hotword weight',
    '识别使用原媒体和所选源音轨。已有主字幕时先预览结果；不会自动翻译或生成配音。': 'Transcribes the original media and selected audio track. Existing subtitles require preview before applying. Translation and TTS run separately.',
    '此便携页面不能调用本机处理服务，请通过独立编辑器入口打开。': 'This portable page cannot call local processing services. Use the direct editor entry.',
    '开始识别': 'Start transcription',
    '确认上次提交': 'Confirm previous submission',
    '放弃未确认提交': 'Forget unconfirmed submission',
    '识别已开始，可以继续编辑；关闭面板不会取消任务': 'Transcription started. You can keep editing; closing the panel does not cancel the job.',
    '识别结果预览': 'Transcription result preview',
    '预览结果': 'Preview result',
    '应用到主字幕': 'Apply to main subtitles',
    '受影响主字幕': 'Affected main subtitles',
    '替换主字幕': 'Replace main subtitles',
    '保留副字幕待复核': 'Secondary subtitles kept for review',
    '保留原配音': 'Original voice assets kept',
    '此结果已应用': 'This result has been applied',
    '没有识别到语音；保留现有字幕': 'No speech detected; existing subtitles are retained',
    'ASR 结果已应用，可一次撤销': 'ASR result applied; undo in one step',
    '候选结果已搁置，仍可在任务列表预览或导出': 'Result set aside; preview or export it from the job list',
    '等待识别': 'Queued for transcription',
    '正在识别': 'Transcribing',
    '识别完成': 'Transcription completed',
    '识别失败': 'Transcription failed',
    '服务中断': 'Service interrupted',
    '使用此范围重试': 'Retry this range',
    '范围已恢复；检查当前配置后点击开始识别': 'Range restored. Check settings, then start transcription.',
    'ASR 服务配置已保存，与启动器共用': 'ASR settings saved and shared with Launcher',
    '请先导入包含音轨的媒体': 'Import media with an audio track first',
    '请选择源媒体内的有效时间范围；未改为整段识别': 'Select a valid range within the source; full-media transcription was not selected',
    '选区切穿已有字幕；请调整范围或先扩展到完整字幕边界': 'The range crosses existing subtitles. Adjust it or expand to complete subtitle boundaries.',
    '结果属于其他工程': 'Result belongs to another project',
    '媒体或源音轨已改变': 'Media or source audio track has changed',
    '目标范围内字幕已新增、删除、拆分、移动或编辑，请重新识别': 'Subtitles in this range were added, removed, split, moved or edited. Transcribe again.',
    '需复核': 'Review needed',
    '主字幕已重新识别，副字幕内容已保留，请复核': 'Main subtitles were transcribed again. Secondary text is retained for review.',
    '来源字幕已删除或被 ASR 替换': 'Source subtitle was deleted or replaced by ASR',
    '配音来源副字幕需复核': 'Secondary subtitle used for this voice needs review',
    '来源字幕已修改，配音需复核': 'Source subtitle changed; review this voice asset',
    '波形已取消；可继续编辑': 'Waveform cancelled; you can keep editing',
    '代理已取消；可继续处理源媒体': 'Proxy cancelled; source processing remains available',
    '正在准备媒体分析…': 'Preparing media analysis…',
    '媒体分析已取消': 'Media analysis cancelled',
    '正在读取工具配置…': 'Loading media tool settings…',
    '路径由启动环境设置。': 'The startup environment controls this path.',
    'FFmpeg 与 FFprobe 已就绪。': 'FFmpeg and FFprobe are ready.',
    '未找到完整媒体工具。': 'FFmpeg or FFprobe is missing.',
    '已保存；可重新导入媒体以刷新音轨信息。': 'Saved. Reimport the media to refresh audio track details.',
    '时间选区': 'Time range',
    '识别所选片段': 'Transcribe selected range',
    '精确编辑时间选区': 'Edit time range precisely',
    '清除时间选区': 'Clear time range',
    '清除选区': 'Clear range',
    '清除时间选区（Esc）': 'Clear time range (Esc)',
    '起点（秒）': 'Start (seconds)',
    '终点（秒）': 'End (seconds)',
    '应用范围': 'Apply range',
    '请输入时间轴范围内的有效起止时间。': 'Enter valid start and end times within the timeline.',
    '生成波形': 'Generate waveform',
    '重新生成波形': 'Regenerate waveform',
    '取消波形': 'Cancel waveform',
    '自动波形': 'Auto waveform',
    '取消导入': 'Cancel import',
    '源音轨': 'Source audio track',
    '波形、试听和 ASR 使用的源音轨': 'Source audio track for waveform, playback and ASR',
    '波形已就绪': 'Waveform ready',
    '使用已有波形': 'Using cached waveform',
    '自动波形已关闭；使用占位波形': 'Auto waveform off; showing placeholder',
    '无音轨；可继续编辑或使用 TTS': 'No audio track; editing and TTS remain available',
    '生成播放代理': 'Generate playback proxy',
    '取消代理': 'Cancel proxy',
    '播放代理已就绪': 'Playback proxy ready',
    '正在生成播放代理…': 'Generating playback proxy…',
    '工具配置': 'Tool settings',
    '媒体工具配置': 'Media tool settings',
    'FFmpeg 可执行文件或所在目录': 'FFmpeg executable or directory',
    '留空自动查找': 'Leave blank to auto-detect',
    '媒体已就绪': 'Media ready',
    '清空全部字幕': 'Clear all subtitles',
    '已清空全部字幕': 'All subtitles cleared',
    '占位波形': 'Placeholder waveform',
    ...
    '在播放头处拆分': 'Split at playhead', '移动贴片': 'Moving clip', '裁切起点': 'Trim start', '裁切终点': 'Trim end',
    ...
    '显示方式': 'Display mode', '语言类型': 'Language type', '字幕语言类型': 'Subtitle language type', '主字幕': 'Main subtitle', '副字幕': 'Secondary subtitle', '主字幕语言类型': 'Main subtitle language type', '副字幕语言类型': 'Secondary subtitle language type', '副字幕时波形高度': 'Waveform height with secondary subtitles', '跨轨道吸附': 'Cross-track snapping', '同时选中主副字幕': 'Select main and secondary subtitles together', '绑定时自动同步时长': 'Automatically sync duration when binding', '交换主副字幕': 'Swap main and secondary subtitles', '普通点击以最后点击的轨道为准；点击已绑定字幕时，仅补选它实际绑定的另一条字幕': 'Normal clicks follow the last clicked track; clicking a bound subtitle only adds the other subtitle actually bound to it',
    ...
    '中键拖动': 'Middle-button drag', '行编号': 'Row numbers', '允许拖动指针': 'Drag to move playhead',
  ...
  const EN_ATTR = {
    ...
    '操作帮助': 'Operation guide',
    '打开帮助窗口的操作说明': 'Open the operation guide in the help window',
  ...
  function translateText(value, lang = language) {
    ...
    const bilingualLanguage = /^[\u3400-\u9fff]+ \/ ([A-Za-z -]+)$/.exec(text);
    if (bilingualLanguage) return bilingualLanguage[1];
    const voiceLabel = /^[\u3400-\u9fff·]+ · ([A-Za-z ]+)(?: · (男声|女声))?$/.exec(text);
    if (voiceLabel) return voiceLabel[1] + (voiceLabel[2] ? ' · ' + (voiceLabel[2] === '男声' ? 'Male' : 'Female') : '');
    if (text.startsWith('Current voice：')) return 'Current voice: ' + translateText(text.slice(14), EN);
    const panelLabel = /^(关闭)?(素材库|媒体播放器|字幕编辑器|波形显示器)窗口(标签)?$/.exec(text);
    if (panelLabel) return (panelLabel[1] ? 'Close ' : '') + ({素材库:'asset library',媒体播放器:'media player',字幕编辑器:'subtitle editor',波形显示器:'waveform'})[panelLabel[2]] + (panelLabel[3] ? ' tab' : ' panel');
    const voiceRecords = /^音色创建记录 \((\d+)\)$/.exec(text);
    if (voiceRecords) return `Voice creation history (${voiceRecords[1]})`;
    const waveformProgress = /^解析波形 (\d+)%$/.exec(text);
    if (waveformProgress) return `Generating waveform ${waveformProgress[1]}%`;
    const sourceTrack = /^源音轨 (\d+)(.*)$/.exec(text);
    if (sourceTrack) return `Source audio track ${sourceTrack[1]}${sourceTrack[2]}`;
    const candidates = /^候选字幕 (\d+) 条（显示前 300 条，可导出全部）$/.exec(text);
    if (candidates) return `${candidates[1]} candidate subtitles (showing 300; export includes all)`;
    const candidateCount = /^候选字幕 (\d+) 条$/.exec(text);
    if (candidateCount) return `${candidateCount[1]} candidate subtitles`;
