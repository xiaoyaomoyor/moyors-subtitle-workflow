...
beta.5 本地入口增加 `--engine firered`、`--firered-punc none|ct-punc`、`--alignment-model`、`--alignment-model-path` 与 `--alignment-mode fill|generate`，详见[本地 ASR](LOCAL_ASR.md)。这些是 `generate_subtitle_local.py` 参数，不表示主程序 CLI 自动暴露全部参数。
