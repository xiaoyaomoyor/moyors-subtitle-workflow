...
beta.5 的兼容决策和验证证据见[升级账本](UPSTREAM_SYNC_1.6.0-beta.5.md)，用户操作见[升级指南](BETA5_UPGRADE.md)。`overlay_track` 保持上游公共契约；ASS 库由 `maw/ass_styles.py` 管理，导出使用工程显式选择与任务快照；本地准备入口在 `maw/msw/local_models.py`，识别仍复用 `gui_workflow`。两个 `tools/*.html` 必须随冻结包分发。
