...
def parse_mosp(text: str) -> tuple[list[TimedChar], int, int]:
    ...
    if "schema" in project and project["schema"] != PROJECT_SCHEMA:
        raise AlignmentError(f"不支持的 MOSP 工程版本：{project['schema']!r}")
    extension = project.get("msw")
    if extension is not None and (not isinstance(extension, dict) or extension.get("schema") != "msw.editor.v1"):
        raise AlignmentError("不支持的 MSW 扩展版本")
...
def generate_matched_mosp(
    ...
    project = {"schema": PROJECT_SCHEMA, **project}
