...
- **字幕素材库** ： 素材库支持音频/字幕类型、批次与文字搜索，统一多选描边，可隐藏底部试听栏；字幕块可按批次复制为独立素材，在字幕编辑器修改并从播放头或拖入位置放回，保留间隔、颜色与完整主副配对，冲突跳过并说明。字幕素材支持保存、另存为、恢复与撤销；本批尚未接入 ASR 入库、音频批量 ZIP 和素材重新生成。
