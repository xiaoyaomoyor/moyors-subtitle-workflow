...
日期：2026-09-07  
状态：原始提案；A0+A1、B、C、D0–D4 已落地，实施与验证记录见下文
...
2026-09-08 D 阶段更新：D0 已实现工程保存、素材收集与本机恢复；D1 / D2 通过 `audio_plan.py` 和 `msw-audio-render-core.js` 共用契约夹具，新增 `audio_render.py` 分块浮点混音、`audio_exports.py` 快照任务及下载。支持配音轨／原声混音 WAV、原声音轨选择、源范围、空隙保护／随媒体裁切和全局峰值衰减，界面位于「文件 → 导出音频」。试听重用同一时长、电平与空隙保护规则。用法见 [音频导出](../EDITOR_AUDIO_EXPORT.md)，验证见 [D1 / D2 记录](../TEST_FEEDBACK_EDITOR_D1_D2.md)。

D3 / D4 延续同一个任务队列和音频计划：`video_render.py` 输出 H.264 / AAC MP4，完整兼容流复制，其余按累计帧边界分段编码；超过画面尾部需明确截断或定格延长。`timeline_export.py` 输出可搬移 OTIOZ，独立配音子轨、浮点增益副本、静音副本与原始 TTS、字幕标记和 SRT，原视频可选收集。范围内静音贴片也保留，但不改变空隙保护；官方 OTIO 解析及逐轨求和有独立验证。前端共用导出表单控制器，新增窗口图标；旧源媒体／表情包／FCP 7 导出范围保持。用法见 [视频与剪辑工程](../EDITOR_VIDEO_TIMELINE_EXPORT.md)，证据见 [D3 / D4 记录](../TEST_FEEDBACK_EDITOR_D3_D4.md)。新增可选主／副／双字幕压制，复用启动器 libass 滤镜及样式，并共享范围和空隙映射。尚未承诺外部剪辑软件逐项兼容、小时级性能、淡入淡出编辑或 E 引擎。
...
2026-09-08 接入方式补充（仅方案，尚未安装或实施）：建议采用「MSW 内置适配器 + 可选本地引擎资源包」，并提供手动指定现有资源目录的入口。设置界面、任务提交和结果登记随 MSW；Node 运行时、固定版本的中文包装／底层库、WASM、映射表及按许可收集的音声原始包独立于 `.mosp` 和便携 HTML。可再分发组件预编译进可选包，避免要求普通用户手动运行 npm；AquesTalk 资源保留来源和原许可，不能把 JS 库许可当作 DLL 的许可。首次启用先验证资源完整性和短句合成，再由后台按需启动本地进程；队列可复用、取消可终止该进程，浏览器不承担模拟器合成。

本次重新核对的 [aquestalk.js README](https://github.com/y52en/aquestalk.js)提供 `load(voice)` 的同捆音声入口和 `loadAquesTalk(zip, dll, options)` 的自定义入口，返回 WAV；与指定中文包装的旧调用形状需要在 E0 冒烟测试中适配，不能直接假定最新版本兼容。合成结果仍走现有素材库 → 音频贴片 → 保存／WAV／MP4／OTIOZ 流程；收到已生成的 WAV 后，播放和导出不依赖油库里运行时，再生成才需要安装引擎。资源路径是本机配置，工程记录版本与配方，不打包整套引擎。
