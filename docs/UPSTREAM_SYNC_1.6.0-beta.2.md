...
| E | 进行中 | 版本、文档、源码回归和 Windows 两包完成；本次已授权推送，正在补齐三平台五包 CI 预演 |
...
| E3 五包构建预演 | 阻塞 | Windows 标准／lite 真实构建、归档和运行通过；尚缺 macOS arm64 两包及 Linux AppImage 的本轮原生构建／运行证据。需核准提交并推送候选分支后运行现有 workflow_dispatch；不以旧 beta.1 CI 替代 |
...
| 项目 | 状态 | 处理与证据 |
|---|---|---|
| F1 固化适配与上游关系 | 进行中 | 将 A～E 审查后的结果提交，再以该完整适配树记录上游 beta.2 的第二父提交；全部 22 项处理见上表。不会把 main 的后续版本带入产品分支 |
| F2 五包原生预演 | 待处理 | 推送集成分支并运行 release.yml 的 workflow_dispatch，要求三平台构建及五包完整性、说明校验全部通过 |
| F3 产品分支与标签 | 待处理 | 预演通过后快进 my-feature；main 仅同步上游主线；beta.2 标签固定到已验收产品提交 |
| F4 公开预发布验收 | 待处理 | 标签构建完成后检查 Release 状态、提交、五个安装包、校验和和实际说明；保留桌面／真实模型等未验证边界 |
