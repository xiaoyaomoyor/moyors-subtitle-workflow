...
这份协议定义 MSW Launcher 如何让 OpenAI-compatible LLM 修改字幕文字，同时保证模型不能写入字幕时间。它是工具箱实现契约，不是工程 schema 的替代品；工程结构仍以 [JSON_SCHEMA.md](../json-schema/) 为准。
...
后处理翻译支持回填主字幕与双语行序，编辑器默认仍写副字幕。主字幕替换按稳定 ID 与任务快照检查冲突，保留已有副轨、配音和未知可选字段；仅对文本实际变化的字幕删除旧字词时间码。空白文本及目标语言过滤的条目返回跳过标记，不得以原文回写覆盖已有译文。流式响应按 UTF-8 解码，保持原有输出协议与时间码校验。
