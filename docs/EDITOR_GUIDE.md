...
- 翻译默认写入副字幕，可选替换主字幕；详见[翻译流程](WORKFLOW.md#在编辑器中翻译字幕)。
...
- 服务器版使用本机系统对话框选择目标位置，新建／另存为成功后绑定新文件；后续保存及自动保存写回该文件，TTS 素材自动收集，原媒体可勾选收集。详见[工程保存与恢复](EDITOR_PERSISTENCE.md)。
- 便携 HTML 沿用浏览器文件句柄与下载方式；本机对话框不可用时也可仅下载工程内容。这种下载不复制素材、不建立服务器保存绑定，后续可再次另存为。
- 空白或只有 SRT 的工程可以正常编辑和保存；没有媒体时只是不显示播放器和波形，不影响保存。

## 基础字幕编辑

配音贴片的成品导出位于「文件 → 导出音频」，提供配音轨与原声混音 WAV；后台处理时可继续编辑。选项与保存边界见[音频导出](EDITOR_AUDIO_EXPORT.md)。
