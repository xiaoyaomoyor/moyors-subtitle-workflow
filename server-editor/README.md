...
用工程 JSON 启动时，服务器会优先读取其中的 `media` 绝对或相对工程目录的路径（也可用 `-m` 覆盖）并自动载入媒体；页面右上角会提示「已自动加载媒体：文件名」。引用失效时优先按引用中的文件名寻找工程同目录媒体，再按工程主名查找；已有有效相对引用继续保留，不自动移动素材。保存工程时会保留实际关联媒体，因此下次启动仍会加载。若媒体已移动或不存在，启动会提示用 `-m` 指定新的路径。
