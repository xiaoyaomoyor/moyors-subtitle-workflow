...
## [1.6.0-beta.5] - 2026-09-23

### 🚀 全新特性

- **独立叠加字幕轨** ： SRT 与启动器文本模式 ASS 支持双层字幕导入，第三层明确报错；支持列表与波形编辑、转轨、预览、保存、撤销，以及字幕／时间线／视频导出；保留 MSW 稳定 ID、素材与副轨文字，Shift 多选和 Ctrl/Cmd+Shift 波形选区不变。TTS／翻译暂不以叠加轨为目标。
- **可选 ASS 样式库** ： 新增共享样式、方案和动画设置，预览与导出独立选择；默认保留旧工程样式，明确开启后采用新库，并冻结视频任务样式快照。
- **本地识别与字词对齐** ： Launcher 和编辑器接入 FireRed CPU、可选 ct-punc、Qwen／FireRed 对齐与运行环境 v7；模型按需准备。时间码工具默认仅补主字幕缺失项，其他轨必须显式选择对应音频，支持取消、新工程输出和失败警告。

### ✨ 提升

- **导出与离线检查** ： FCP7 补全原生文字及序列参数；Lottie／OGraf 支持跟随媒体和自定义尺寸；新增转写文本与字词时间码两个离线对比工具，保留上游嵌入报告兼容。
- **编辑与恢复** ： 改稿后的字词拆分保持顺序并提示漂移，字体支持搜索与键盘选择；同工程同地址断线恢复不重复开页，调试产物按 MSW 输出布局归档。

- **启动器混合输入、模块配置与统一输出** ： 预制工程改为一个可拖入、点击多选的文件队列，统一处理媒体、工程与 SRT/ASS；同名媒体和字幕需确认后关联，逐任务选择音轨及文稿/OCR 来源。新增统一预检、顺序执行、取消、跳过已完成任务的重试与草稿恢复，已有工程生成派生文件并保留编辑器扩展内容。右栏成为模块唯一启用入口，输入以外默认折叠，序号和布局保持一致；仅 SRT 与可选便携 HTML 收入输出模块。校对、断句和翻译可独立选择已配置供应商并查看预设提示词，译文可写入副字幕、回填或合并双语；原文、译文和双语 SRT 分别发布，原文自定义路径与工程目录独立。波形支持复用／重建有效缓存，实用工具新增独立的生成波形入口，保留工程扩展与音频素材，支持重关联、音轨、频谱、取消及打开派生工程。 界面语言移入「更多设置 → 外观与语言」，与识别语言列表分开；统一工具/设置栏缩进、可聚焦的说明浮层、窄窗键盘焦点和减少动态效果，切换语言保留队列与配置草稿。

### 🐛 修复

- **旧工程接管兼容** ： 从导入文件校验原始字幕内容，避免编辑器补齐的轨道默认设置阻止服务器接管；继续核对所有字幕轨及权威帧时间，拒绝同名不同内容的工程。
- **识别时间码与短句边界** ： Qwen/Fun-ASR 的少量零宽词或缺失句不再废弃其余有效时间码；短句合并先检查最大字数／词数，避免合并后又拆分。
