...
class JobManager:
    ...
    def __init__(self, path: Path, *, translate=translate_snapshot, test_connection=test_llm_connection):
    ...
    def _get(self, job_id: str, project_id: str) -> dict:
    ...
    def submit(self, payload: dict, settings: LlmSettings) -> dict:
        ...
        if kind not in {"translation", "connection_test"}:
            ...
            try:
                ...
                def progress(stage, details):
                    with self.lock:
                        if cancel.is_set():
                            raise JobCancelled()
                        current = self._get(job_id, project_id)
                ...
                with self.lock:
                    if cancel.is_set():
                        raise JobCancelled()
                    current = self._get(job_id, project_id)
                    current.update(status="succeeded", result=result, stage="done")
                    self._write(current)
            except JobCancelled:
                with self.lock:
                    current = self._get(job_id, project_id)
