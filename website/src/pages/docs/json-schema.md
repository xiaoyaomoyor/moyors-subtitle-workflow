...
恢复草稿和按分钟采样的保存历史写入本机 `editor-recovery.sqlite3`，不属于工程契约，不随工程搬移；快照不保存可重建波形或音频字节。读取历史快照时同样验证两个工程版本，旧快照只在返回的内存副本补顶层版本，不改写数据库原记录；未知版本在登记恢复来源前拒绝。详见[保存与恢复](https://github.com/xiaoyaomoyor/moyors-subtitle-workflow/blob/my-feature/docs/EDITOR_PERSISTENCE.md)。
