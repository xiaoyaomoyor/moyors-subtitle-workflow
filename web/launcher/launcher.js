(function () {
  ...
  const STRINGS = {
    zh: {
      ...
      summary_auto_video: "跟随工程视频",
      summary_target_zh: "目标 中文",
      summary_target_en: "目标 英文",
      summary_default_output: "默认输出策略",
      ...
      auto_enable_step_resegment: "启用重新断句",
      auto_enable_step_ocr: "启用 OCR 字幕去重",
      auto_enable_step_translate: "启用字幕翻译",
      mod_waveform_hint: "为媒体生成波形缓存供编辑器显示；不生成字幕。已有有效缓存会直接复用，媒体或源音轨变化后自动重新生成。",
      alignment_manual_badge: "需人工确认",
      alignment_card_hint: "根据文稿整理口播素材，在对齐工作台中选择需要保留的录制片段，再导出对齐后的工程。",
      alignment_card_follows: "工程引用与校对文稿在实用工具页的口播对齐面板中配置（默认跟随当前工程）。",
      alignment_open_workbench: "打开对齐工作台",
      alignment_use_result: "选择对齐结果工程…",
      alignment_waiting_manual: "已打开对齐工作台：人工选择保留片段并导出后，回到这里接入结果。",
      alignment_result_attached: "已接入对齐结果工程。",
      postprocess_provider_hint: "校对/断句/翻译共用的后处理 LLM 供应商（与上方保持同步）。",
      toolbox_prompt_proofread: "校对提示词（追加）",
      toolbox_prompt_resegment: "断句提示词（追加）",
      toolbox_prompt_translate: "翻译提示词（追加）",
      rail_log_toggle: "处理日志",
      ...
      prefab_input_mode_label: "输入方式",
      prefab_rail_label: "处理模块",
      prefab_mode_media: "媒体预制",
      prefab_mode_project: "已有工程／字幕再处理",
      project_card_title: "输入工程／字幕",
      project_input_label: "工程或字幕文件",
      project_drop_hint: "拖入 .mosp / .json 工程或 .srt 字幕，或点击选择。",
      rail_input: "输入与分析",
      rail_subtitle: "字幕整理",
      rail_language: "语言处理",
      rail_advanced: "高级",
      rail_feedback: "运行反馈",
      rail_show_log: "显示处理日志…",
      rail_fixed_note: "必需",
      mod_media: "选择媒体",
      mod_waveform: "波形生成",
      mod_asr: "识别设置（ASR）",
      ...
      project_input_required: "请先选择要处理的工程或字幕文件。",
      project_input_unsupported: "该输入模式只支持 .mosp 工程或 .srt/.ass 字幕文件。",
      prefab_no_steps: "没有可执行的步骤：请至少启用一个字幕处理模块。",
      prefab_srt_needs_media: "{modules} 需要视频画面，SRT 输入没有媒体；请改用工程输入或补充媒体。",
      prefab_plan_running: "正在按方案处理工程／字幕…",
      prefab_plan_done: "处理完成：{path}",
      prefab_plan_done_short: "方案处理完成",
      chip_input_project: "工程输入",
      chip_input_srt: "SRT 输入",
      chip_translate: "翻译副轨",
      chip_alignment_manual: "口播对齐（人工）",
    ...
    en: {
      ...
      summary_auto_video: "Follows project video",
      summary_target_zh: "Target Chinese",
      summary_target_en: "Target English",
      summary_default_output: "Default output policy",
      ...
      auto_enable_step_resegment: "Enable re-segmentation",
      auto_enable_step_ocr: "Enable OCR dedup",
      auto_enable_step_translate: "Enable subtitle translation",
      mod_waveform_hint: "Generate a waveform cache for the editor; no subtitles. Valid caches are reused; media or track changes regenerate automatically.",
      alignment_manual_badge: "Manual step",
      alignment_card_hint: "Organize narration takes from the manuscript, pick the keeps in the alignment workbench, then export the aligned project.",
      alignment_card_follows: "Project reference and manuscript are configured in the Speech alignment panel on the Tools page (follows the current project).",
      alignment_open_workbench: "Open alignment workbench",
      alignment_use_result: "Attach aligned project…",
      alignment_waiting_manual: "Alignment workbench opened: pick keeps and export there, then attach the result here.",
      alignment_result_attached: "Aligned project attached.",
      postprocess_provider_hint: "Shared post-processing LLM provider for proofread/resegment/translate (synced with the one above).",
      toolbox_prompt_proofread: "Proofreading prompt (append)",
      toolbox_prompt_resegment: "Re-segmentation prompt (append)",
      toolbox_prompt_translate: "Translation prompt (append)",
      rail_log_toggle: "Processing log",
      ...
      prefab_input_mode_label: "Input mode",
      prefab_rail_label: "Processing modules",
      prefab_mode_media: "From media",
      prefab_mode_project: "From project / subtitles",
      project_card_title: "Project / subtitle input",
      project_input_label: "Project or subtitle file",
      project_drop_hint: "Drop a .mosp / .json project or .srt subtitle, or click to choose.",
      rail_input: "Input & analysis",
      rail_subtitle: "Subtitle cleanup",
      rail_language: "Language",
      rail_advanced: "Advanced",
      rail_feedback: "Run feedback",
      rail_show_log: "Show processing log…",
      rail_fixed_note: "required",
      mod_media: "Select media",
      mod_waveform: "Waveform",
      mod_asr: "Recognition (ASR)",
      mod_match: "Script match",
      mod_replace: "Fixed processing",
      mod_proofread: "LLM proofread",
      ...
      project_input_required: "Choose a project or subtitle file to process first.",
      project_input_unsupported: "This input mode accepts .mosp projects or .srt/.ass subtitle files only.",
      prefab_no_steps: "No steps to run: enable at least one subtitle-processing module.",
      prefab_srt_needs_media: "{modules} needs video frames; SRT input has none. Use a project input or attach media.",
      prefab_plan_running: "Processing project/subtitles per plan…",
      prefab_plan_done: "Done: {path}",
      prefab_plan_done_short: "Plan processing finished",
      chip_input_project: "Project input",
      chip_input_srt: "SRT input",
      chip_translate: "Translation track",
      chip_alignment_manual: "Alignment (manual)",
  ...
  Object.assign(STRINGS.zh, {
    ...
    advanced_misc: "其他",
    generate_spectral: "生成频谱颜色数据",
    generate_spectral_hint: "默认只生成波形；勾选后会额外计算频谱，耗时和文件体积都会增加。",
    generate_spectral_title: "为独立波形缓存增加频谱颜色数据。",
    segmentation: "字幕切句",
    max_len: "最大字数",
    min_len: "短句合并阈值",
    max_words: "英文最大单词数",
    ...
    doubao_options_title: "豆包即时热词", doubao_hotwords: "即时热词（每行一个）",
      subtitle_invalid: "无法解析字幕或工程", script_invalid: "无法读取文稿", match_invalid: "文稿匹配设置无效", match_too_low: "文稿匹配覆盖率不足",
      openai_timestamps_required: "接口必须返回可靠时间戳；Keywords 每行一个",
      openai_timestamps_missing: "此模型在当前接口没有可靠字幕时间戳，请更换模型",
  ...
  Object.assign(STRINGS.en, {
    ...
    advanced_misc: "Other",
    generate_spectral: "Generate spectral color data",
    generate_spectral_hint: "By default only the waveform is generated. Spectral data adds processing time and file size.",
    generate_spectral_title: "Add spectral color data to the waveform cache.",
    ...
    doubao_options_title: "Doubao hotwords", doubao_hotwords: "Hotwords (one per line)",
      subtitle_invalid: "Cannot parse subtitles or project", script_invalid: "Cannot read manuscript", match_invalid: "Invalid match settings", match_too_low: "Manuscript match coverage is too low",
      openai_timestamps_required: "The endpoint must return reliable timestamps. One keyword per line.",
      openai_timestamps_missing: "This model has no reliable subtitle timestamps on this endpoint. Choose another model.",
  ...
  function mockApi() {
    ...
    return {
      ...
      choose_file: async ({ kind, multiple }) => {
        // R4：批量「添加文件」走 multiple 媒体选择，返回两个不同媒体供队列测试。
        if (kind === "media" && multiple) return { ok: true, paths: ["D:\\Demo\\a.mp4", "D:\\Demo\\b.mp4"] };
        return { ok: true, path: kind === "json" ? "D:\\Demo\\project.json" : (kind === "subtitle" ? "D:\\Demo\\project.mosp" : (kind === "subtitle-burn" ? "D:\\Demo\\clip.srt" : (kind === "video" ? "D:\\Demo\\clip.mp4" : (kind === "ffconcat" ? "D:\\Demo\\clip.ffconcat" : (kind === "script" ? "D:\\Demo\\script.txt" : (kind === "hotwords" ? "D:\\Demo\\hotwords.txt" : "D:\\Demo\\clip.mp4")))))) };
      },
  ...
  save_local_settings: async ({ runtimePath }) => { state.config.localRuntime = { ...(state.config.localRuntime || {}), path: runtimePath || "D:\\Users\\Demo\\AppData\\Local\\MSW\\local-runtime" }; return { ok: true, runtimePath: state.config.localRuntime.path, runtime: state.config.localRuntime }; },
      ...
      choose_folder: async ({ kind } = {}) => ({ ok: true, path: kind === "model-cache" ? "D:\\Models\\MSW" : (kind === "ocr-runtime" ? "D:\\Models\\MSW\\ocr-runtime" : (kind === "runtime" ? "D:\\Users\\Demo\\AppData\\Local\\MSW\\local-runtime" : "D:\\Stickers")) }),
      ...
      run_prefab_plan: async ({ plan }) => {
        (window.__prefabPlanRuns = window.__prefabPlanRuns || []).push(JSON.parse(JSON.stringify(plan)));
        const input = String((plan && plan.input && plan.input.path) || "");
        const taskId = "mock-prefab";
        const projectPath = input.replace(/\.(mosp|json|srt|ass)$/i, ".processed.mosp");
        setTimeout(() => {
          window.MSWLauncher.onBackendEvent({ type: "prefabTask", taskId, status: "running", inputPath: input });
          window.MSWLauncher.onBackendEvent({ type: "prefabTask", taskId, status: "completed", projectPath, srtPath: "", translatedSrtPath: "", warnings: [] });
        }, 60);
        return { ok: true, taskId, status: "running", inputKind: input.toLowerCase().endsWith(".srt") ? "srt" : "project" };
      },
      cancel_prefab_plan: async () => ({ ok: true, cancelled: true, taskId: "mock-prefab" }),
      start_batch_projects: async ({ items, plan }) => {
        (window.__batchPlanRuns = window.__batchPlanRuns || []).push({ items: JSON.parse(JSON.stringify(items)), plan: JSON.parse(JSON.stringify(plan)) });
        setTimeout(() => {
          window.MSWLauncher.onBackendEvent({ type: "batchStarted", total: items.length, manifestPath: "" });
          items.forEach((item, index) => {
            window.MSWLauncher.onBackendEvent({ type: "batchItem", id: item.id, index, status: "running", mediaPath: item.mediaPath });
            window.MSWLauncher.onBackendEvent({
              ...
              projectPath: item.mediaPath.replace(/\.[^.]+$/, (plan && plan.modules && plan.modules.waveform === false) ? ".media.mosp" : ".waveform.mosp"),
              jsonPath: item.mediaPath.replace(/\.[^.]+$/, (plan && plan.modules && plan.modules.waveform === false) ? ".media.mosp" : ".waveform.mosp"),
            });
          });
          window.MSWLauncher.onBackendEvent({ type: "batchDone", status: "complete", outcomes: [], manifestPath: "" });
        }, 60);
        return { ok: true, itemCount: items.length, waveform: !(plan && plan.modules && plan.modules.waveform === false) };
      }    };
  }
  ...
  function runtimeHintText(runtime, readyKey, otherKey) {
    ...
    if (runtime.status === "broken" || runtime.error || runtime.diagnostic) return [t(fallbackKey), runtime.detail, runtime.error, runtime.diagnostic].filter(Boolean).join("\n");
    return ["ready", "missing", "installing", "checking"].includes(runtime.status) ? t(fallbackKey) : (runtime.detail || t(fallbackKey));
  }
  ...
  function errText(code, detail, context = {}) {
    if (["subtitle_invalid", "script_invalid", "match_invalid", "match_too_low"].includes(code)) return t(code);
  ...
  function handlePrefabTaskEvent(event) {
    // R4/F05：工程／SRT 输入的方案执行任务；与波形/转录任务状态互不干扰。
    if (event.taskId && state.prefabTaskId && event.taskId !== state.prefabTaskId) return;
    if (event.status === "running") { setStatus(t("prefab_plan_running")); return; }
    state.prefabTaskId = "";
    setRunning(false);
    if (event.status === "completed") {
    ...
    if (event.status === "failed") {
      ...
      showErrorNotice(message, event.code || "postprocess_failed", event.detail || event.error || "");
      if (event.postprocessRunDirectory) $("retryPostprocess")?.classList.remove("hidden");
    }
  }
  ...
  function renderLocalRuntime() {
    ...
    button.disabled = !installing && (state.running || state.batchRunning || state.localPreparing);
  ...
  function localRuntimeBusy() { return state.running || state.batchRunning || state.localPreparing || state.localRuntimeInstalling || state.config?.localRuntime?.status === "installing"; }
  function syncLocalRuntimeControls() {
    const busy = Boolean(localRuntimeBusy());
    const moss = selectedModel()?.engine === "moss";
    $("localRuntimePath").disabled = busy || moss;
    $("pickLocalRuntimePath").disabled = busy || moss;
    $("localRuntimePath").closest(".field").classList.toggle("hidden", moss);
  }
  async function saveLocalRuntimePath(path) {
    if (localRuntimeBusy() || selectedModel()?.engine === "moss") {
      const result = { ok: false, field: "localRuntimePath", code: "local_runtime_busy", detail: state.lang === "en" ? "Wait for the current task to finish before changing the runtime directory." : "请等待当前任务结束后再切换运行环境目录。" };
      applyErrorResult(result); return result;
    }
  ...
  function syncOpenaiCapabilities(reset=false) {
    ...
    const name=isCustomOpenAiModel()?$("openaiModel").value.trim():$("model").value;
    const model=provider().models.find(m=>m.id===name.replace(/^openai\//,''))||selectedModel();
    const caps=model.openaiCapabilities?.[family];if(!caps)return;
    if(reset)$("openaiDiarize").checked=caps.diarize;
    if(caps.diarize)$("openaiDiarize").checked=true;
    const diarize=$("openaiDiarize").checked;
    if(diarize){$("openaiPrompt").value='';$("openaiKeywords").value='';}
    $("openaiPrompt").closest('.field').classList.toggle('hidden',!caps.prompt||diarize);
    $("openaiKeywords").closest('.field').classList.toggle('hidden',!caps.keywords||diarize);
    $("openaiDiarize").closest('label').classList.toggle('hidden',!caps.diarize&&!caps.customDiarize);
  ...
  function applyZoomPercent(value) { const zoomPercent = normalizeZoomPercent(value); document.documentElement.style.zoom = `${zoomPercent}%`; document.documentElement.style.setProperty("--launcher-shell-height", `${100 / (zoomPercent / 100)}dvh`); state.config.zoomPercent = zoomPercent; window.dispatchEvent(new Event('launcherzoomchange')); return zoomPercent; }
  ...
  function getDefaultAudioTrackForMedia(path) {
    if (audioTrackPathKey(path) !== state.audioTrackPath || !state.audioTracks.length) return null;
    return audioTrackIndex(state.audioTracks.find(audioTrackIsDefault) || state.audioTracks[0]) ?? 0;
  }
  ...
  $("outputSubfolder").addEventListener("change", async () => { await saveOutputPref("outputSubfolder"); $("perVideoSubfolder").disabled = !state.config.outputSubfolder; syncDefaultOutputPreview(); });
  ...
  document.addEventListener("DOMContentLoaded", () => {
    void init().catch((error) => {
      ...
      appendLog(`[init-stack] ${String(error && error.stack).split("\n").slice(0, 4).join(" | ")}`);
      setStatus(message);
      revealLauncher();
    });
  });
