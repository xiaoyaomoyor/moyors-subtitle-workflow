...
- **双列表头单行化** ： 双列（主/副字幕）表头不再换行成两行——移除表头的 flex-wrap，改为时间盒收缩截断兜底；双列每列只有容器一半宽，紧凑时间与「主/副」单字前缀在容器 ≤940px 提前启用（单列仍为 460px），字数退场阈值不变（420px）。e2e 在 11 档宽度断言表头单行、元素同行不重叠、不溢出列盒。
- **字幕编辑器提示行键帽对齐** ： 字幕编辑器提示行的 kbd 此前被一条后期规则压成 10.5px 小号扁键帽（内边距 1px 4px、无投影），现回落到与其他三个模块提示行完全相同的共享键帽样式（13px 等宽、2px 6px、底部微投影），提示间距同步对齐（10px/5px）。
