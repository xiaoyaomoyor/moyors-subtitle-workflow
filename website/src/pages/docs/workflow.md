...
- 输出默认选择「生成／更新副字幕」；可切换「替换主字幕」。替换只更新仍与任务快照相符的主字幕文本，保留稳定 ID、副轨与配音贴片，清除已改文字幕的旧字词时间码，并支持一次撤销。目标语言过滤跳过的条目不会覆盖已有译文。
...
- 处理期间可继续编辑、播放和拖动时间，关闭翻译面板不取消任务。完成后自动应用仍有效的结果；文本、绑定或轨道有变化的条目留下提示，可查看和复制译文，或在恢复到兼容状态后点击「检查并应用」。一次撤销会撤回本次实际写入的字幕。
...
在「预制工程 → 输入媒体/工程/字幕」点击虚线框多选，或拖入媒体、`.mosp`/`.json` 工程、SRT/ASS。不再切换单文件或批量模式。重复路径会忽略；不支持、损坏或失联的文件显示具体原因。ASS 只导入时间与文本，不导入样式、特效和绘图。

同目录同名媒体和字幕显示关联候选；选择「关联为一个任务」后共同生成工程，也可分别处理或解除关联。因此文件数与任务数可能不同。展开条目详情可查看路径、选择多音轨、重新关联媒体，以及填写本任务文稿或 OCR 视频。

右侧勾选决定是否执行模块；左侧折叠只改变显示。输入默认展开，其余默认折叠。识别默认只处理没有字幕的任务；已有字幕可显式选择重新识别，写入新工程。字幕输入和已有工程可以只处理文字；波形与识别需要可用媒体音轨。多任务文稿匹配必须逐项指定文稿，不会静默跳过或把一份文稿套给整批。

点击「制作工程」后先统一预检，再按队列顺序执行冻结后的方案。预检未通过时定位首个有问题的模块；运行中单个任务失败不影响后续任务。「停止」取消当前与未开始的任务，已完成结果保留。再次制作时可跳过已完成任务；失败任务重新执行其处理链，不是步骤级续传。

结果默认写入新的派生工程，同名自动避让，不覆盖输入。队列条目可打开结果工程或所在文件夹。「输出」中可选择只生成 SRT，或附带便携 HTML；仅 SRT 时不发布工程，也不生成波形和 HTML。关闭输出模块时采用默认输出配置，其隐藏自定义草稿不生效。

队列路径、关联、逐任务选择和已完成结果保存在本机草稿中，重新打开时重新检查文件；密钥不写入队列草稿。新队列不再创建旧版 `maw-batch-manifest.json`。供应商和模型等公共调用设置仍共用，后处理遵循[处理管线](https://github.com/xiaoyaomoyor/moyors-subtitle-workflow/blob/my-feature/docs/POSTPROCESS_PIPELINE.md)。
...
编辑时可在「波形显示器设置」开启「波形跟随源试听增益」（默认关闭）。设置旁和波形工具栏会显示源音频试听增益的 dB 值；幅度变化仅重绘可见行，复用现有峰值缓存，无需重新解析音频。增益过大时波峰按显示区域截顶；这不是音频文件已经削波的判定。

「字幕 → 缩放/偏移字幕」支持明确选择主轨／当前副轨、所选字幕／整条轨道，以及是否联动绑定字幕。范围为空时不自动改为全部。字词时间随字幕调整，连续滚轮或长按合为一次撤销，结果和边界限制显示在面板底部；快捷键说明位于标题旁的提示图标。

新建工程的保存窗口称为「新工程保存位置」，已有工程副本仍使用「另存为工程」。手动保存先显示「正在保存」及旋转图标，成功后同一气泡变绿并退去；保存期间继续编辑会提示新改动尚未保存。自动保存成功保持静默，失败仍提示。便携页面的浏览器原生文件选择器标题由浏览器控制。
...
- 校对、重新断句、翻译分别选择已在更多设置中保存的供应商，旁边显示该服务已保存的模型。更多设置的供应商选择只决定正在编辑哪一份环境，不改三个模块的调用服务。预设提示词可展开查看，追加提示词单独填写；翻译目标切换时同步切换预设。
- 翻译的“译文写入”三选一：生成副字幕保留主字幕并新增译文轨；翻译回填替换主字幕文本；合并双语把原文和译文写入每条主字幕，可选择上下顺序。三种方式均保留原有副轨、配音贴片与素材；已有主副绑定不被新译文抢占。
- 输出中的原文 SRT 是文稿匹配、固定处理、校对、断句、OCR 后、翻译前的快照。译文 SRT 仅含译文；双语 SRT 仅在合并双语模式提供。三个开关独立，旧双语配置首次显示迁移提示。
- “自定义原文 SRT 位置”仅单任务展开使用，与工程输出目录独立；批量采用目录和文件名规则。所有派生产物避免覆盖已有输入/输出，冲突时按组加序号。输出模块关闭后隐藏草稿不参与执行，再次开启仍能继续编辑。动作区显示预计产物，实际文件以完成结果为准。
- “只生成 SRT”不发布最终工程或 HTML，波形暂不适用但保留配置草稿。需要已有可导出的字幕或开启 ASR，且至少选择一种可输出的 SRT。空识别结果不会显示空产物成功。
- 预制工程的波形模块可选择优先复用有效缓存或显式重新生成；音轨来自各任务详情。缓存校验来源签名、音轨和版本，关闭频谱不删除仍有效的数据。
- 实用工具 → 工程辅助 → 生成波形：选择媒体创建空字幕工程，或选择已有工程补波形/频谱；媒体失联时可重新关联。工具自有输入、音轨、输出目录和取消按钮，不依赖预制队列；始终另存派生工程，完成后可打开工程或所在文件夹。频谱或某一缓存层失败会显示具体提示，原有有效缓存与源工程保留。
- 中间产物保留选项位于更多设置的缓存与诊断区域。模块长说明收纳到名称旁的 ⓘ，悬浮、聚焦或点击查看，Esc/点击外部关闭；错误、必填条件与执行进度直接显示。


## 启动器外观、语言与帮助（E+F）

- 界面语言入口位于「更多设置 → 外观与语言 → 界面语言」，提供简体中文与 English。下方「识别语言列表」只控制 ASR 语种选项，两者互不影响。
- 语言/主题切换和页面导航保留本次队列、模块启用、服务选择、提示词及收展状态；默认收起只在新打开启动器时生效。
- 长说明使用名称旁的 ⓘ；鼠标悬浮、键盘聚焦均可阅读，点击可保持展开，再次点击、Esc 或点击外部关闭。浮层不会被模块裁切，适配窗口缩放。
- 工具与设置右栏按分类缩进；窄窗用「选择工具／选择分组」打开。方向键切换，Esc 收起并返回入口按钮；减少动态效果的系统偏好会关闭模块展开动画。
- E/F 验证与原生宿主边界见 [实施记录](https://github.com/xiaoyaomoyor/moyors-subtitle-workflow/blob/my-feature/docs/IMPLEMENT_LAUNCHER_EF_20260922.md)。
