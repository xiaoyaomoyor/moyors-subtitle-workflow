...
编辑器可直接加载原视频，后台解析波形并从「媒体 → ASR」发起整段／片段识别，无需先在 Launcher 生成工程。发行包使用独立启动脚本，源码使用 `uv run python maw_gui.py --editor --blank`；空工程也可手动编辑和纯 TTS。详见 [直接导入与 ASR](https://github.com/xiaoyaomoyor/moyors-subtitle-workflow/blob/my-feature/docs/EDITOR_ASR.md)。
...
- Windows 图形版：双击 Release 压缩包中的 `MSW.exe` 生成工程；完成后默认使用 Server 版编辑器，便携编辑器 HTML 仅保留兼容入口。

无论用哪种方式，`.mosp` / `.json` 工程文件都是字幕真源；新工程优先使用 `.mosp`，旧 `.json` 工程无需迁移即可继续编辑。SRT 只能保留文本和时间，不能完整保留字/词级时间码、表情包、颜色、波形、工作区与静音空隙决定。

### beta.4 编辑器变化

- 波形的响度适配使用有效 RMS 缓存自动调整显示振幅，新工程默认开启；旧 MSW 工程保留手动振幅，上游工程显式保存的自动／手动值继续生效。手动调节后进入手动模式，点击响度适配可恢复自动。仅有峰值或尚未解析响度时保留当前振幅；源试听增益可独立叠加显示。
- 字幕列表可控制是否跟随播放；滚轮、触摸和列表导航优先保留正在阅读的位置。拆分、合并、撤销和保存尽量保持可见字幕位置，保存不会结束正在输入的文字。
- ASS 按媒体分辨率调整字号，保留字幕颜色及可选说话人名称；支持去空隙 ASS。缺少视频尺寸时按 1920×1080 导出。
- 颜色组可将单条字幕脱离并继承当前组颜色；中途改色会拆开原组。共同边界仍沿用 MSW 的双手柄联动规则。
- 翻译默认写入副字幕，可选替换主字幕；详见[翻译流程](../workflow/#在编辑器中翻译字幕)。
...
- 服务器版使用本机系统对话框选择目标位置，新建／另存为成功后绑定新文件；后续保存及自动保存写回该文件，TTS 素材自动收集，原媒体可勾选收集。详见[工程保存与恢复](https://github.com/xiaoyaomoyor/moyors-subtitle-workflow/blob/my-feature/docs/EDITOR_PERSISTENCE.md)。
- 便携 HTML 沿用浏览器文件句柄与下载方式；本机对话框不可用时也可仅下载工程内容。这种下载不复制素材、不建立服务器保存绑定，后续可再次另存为。
- 空白或只有 SRT 的工程可以正常编辑和保存；没有媒体时只是不显示播放器和波形，不影响保存。

## 基础字幕编辑

配音贴片的成品导出位于「文件 → 导出音频」，提供配音轨与原声混音 WAV；后台处理时可继续编辑。选项与保存边界见[音频导出](https://github.com/xiaoyaomoyor/moyors-subtitle-workflow/blob/my-feature/docs/EDITOR_AUDIO_EXPORT.md)。
...
普通移动和键盘边界微调是否默认联动，由「波形显示器设置 → 操作 → 自动吸附调整相邻字幕」控制，默认开启；Alt 临时反转该设置。鼠标边缘拖动按点亮的手柄决定：靠近某一真实边缘只点亮该侧，同轨两条字幕精确相接且靠近共同边界时同时点亮左块右手柄、右块左手柄，拖动共同调整。只点亮一个手柄时仅调整该侧，不带动同轨邻居；Alt 可临时只点亮并拖动一侧。真实共同边界可以位于换行处，字幕跨行显示的续接位置没有手柄。主／副字幕的绑定跟随规则独立于同轨边界规则。`current-cue-panel` 文本框按 `Esc` 默认保留文本改动并退出编辑；打开该面板齿轮中的「操作 → Esc 取消编辑」后，`Esc` 会恢复进入本次编辑前的文本。`Enter` 或 `Ctrl+Enter` 的保存/拆分语义不变。

多重字幕开启时，如果当前只选中一条副字幕，`B` 会直接打开该副字幕的拆分流程；在字幕列表中悬停已选副字幕时，`B` 会按鼠标所在的文字断点打开弹窗，并遵循副字幕的“单词型 / 字符型”设置。按 `↑` / `↓` 可在当前时间范围对应的主字幕和副字幕之间切换焦点；有绑定时优先切换到绑定项，没有绑定时选择时间重叠或距离最近的一条。按 `Shift` 在波形空白处拖动框选时，主轨和副轨都会参与命中。按 `G` 可把当前副字幕绑定到主字幕，按 `Shift+G` 解绑，按 `H` 将已绑定副字幕对齐到主字幕时间轴。`G` 在同时选中一条主字幕时直接绑定，否则会自动拾取时间重叠且尚未绑定的主字幕中时间最早的一条；如果所有重叠主字幕都已有绑定，才进入等待手动选择流程。绑定时默认自动执行一次 `H`，可在多重字幕齿轮中关闭“绑定时自动同步时长”。副字幕右键菜单中的「在鼠标位置拆分」「绑定到主字幕」「解绑」「对齐主字幕时间范围」分别标注 `B` / `G` / `Shift+G` / `H`。普通点击以最后点击的轨道为准：点击未绑定副字幕会清除旧主字幕选区；开启“同时选中主副字幕”时，点击已绑定字幕只补选它实际绑定的另一条字幕。显式 Ctrl/Shift 多选仍可保留主副字幕作为有意的绑定/替换选择。「字幕 → 多重字幕设置」中可切换主字幕语言类型、副字幕语言类型、跨轨道吸附、同时选中绑定的主副字幕、绑定时自动同步时长、显示轨道徽标和交换主副字幕；“同时选中主副字幕”在新设置中默认关闭（保留已有保存值），“绑定时自动同步时长”默认开启，“显示轨道徽标”默认关闭，编辑区仍以最后点击的字幕为准。单词型适合英语等按空格拆分的语言，字符型适合中文、日文等按字符拆分的语言。仅在主字幕或副字幕单独显示时，列表字数和超长筛选按对应轨道的语言类型计数；双列／双排卡片按各侧语言类型显示字数，可在字幕列表设置隐藏，当前字幕面板仍按最后点击字幕的语言类型计数。跨轨道吸附默认开启，拖动字幕或边界时会把另一条轨道的起止位置加入吸附目标。开启多重字幕时使用齿轮中的「副字幕时波形高度」（默认 168px），关闭后恢复「配置」中的波形高度。共享波形双 lane 的轨道徽标默认关闭，开启后 `1` / `2` 分别表示主轨 / 副轨。
...
MSW 的工程文件会保存 `segments[*].items` 字/词级时间码。存在这些数据时，拆分后两段会按字/词边界重新分配时间，而不是粗略平分，因此时间仍然准确。没有 `items` 的外部 JSON 也能打开，但拆分时只能按字符比例估算，精度会下降。
...
在 localhost 编辑器中，“保存工程”（`Ctrl(Cmd)+S`）会原子写回当前 `.mosp` / `.json`，并保留同目录 `.bak`。“另存为工程”（`Ctrl(Cmd)+Shift+S`）可通过系统对话框选择新位置，自动收集 TTS 音频，可勾选收集原媒体；成功后继续写回新工程。弹窗打开期间暂停自动保存，保存期间的新修改仍保持未保存状态。文字编辑提交后的短暂防抖与常规定时自动保存继续沿用设置。便携 HTML 使用“另存为工程”的浏览器保存／下载路径；服务不可用时也可仅下载工程数据，但音频文件需要另行保留。文件菜单的“恢复记录”可以找回本机草稿与历史副本，详见[工程保存与恢复](https://github.com/xiaoyaomoyor/moyors-subtitle-workflow/blob/my-feature/docs/EDITOR_PERSISTENCE.md)。空白 localhost 编辑器打开或拖入工程时，继续尝试按已知媒体位置和一致文件内容接管工程；无法接管时提示选择关联媒体。

- **工程文件（`.mosp` / `.json`）**：继续编辑时优先保存它；新工程建议使用 `.mosp`，`.json` 用于兼容旧工程和已有工作流。
- **WAV / MP4 / 配音 OTIOZ**：文件菜单可打开音频和视频导出窗口；「导出字幕 → 更多导出 → OTIO」中可导出带独立配音片段的剪辑素材包。操作与边界见[音频导出](https://github.com/xiaoyaomoyor/moyors-subtitle-workflow/blob/my-feature/docs/EDITOR_AUDIO_EXPORT.md)及[视频与剪辑工程导出](https://github.com/xiaoyaomoyor/moyors-subtitle-workflow/blob/my-feature/docs/EDITOR_VIDEO_TIMELINE_EXPORT.md)。
...
- **Resolve JSON**：保存字幕、颜色、表情包与媒体的批量导入数据。它是导出用的交换文件，不是 MSW 工程文件；MSW 只负责导出这个数据文件，要实际操作达芬奇仍需在达芬奇环境中运行兼容的执行脚本。
...
- 可直接导入视频，在编辑器内完成波形解析、整段/多片段 ASR、翻译与配音；允许 0 字幕和 0 音频贴片。源媒体 OTIO/OTIOZ 与含配音 OTIOZ 使用不同导出入口。
- 全局设置保留 MSW 搜索/主题；保存分类可配置磁盘版本间隔和数量，恢复版本打开未保存副本。
- 说话人分类可启用五色姓名映射；预览有下划线、文字、阴影、描边样式，普通/副轨/分色 SRT 共用可选姓名输出。
- 主副轨交换保留稳定 ID、绑定和声音素材；字数过滤可选择主轨、副轨或任一轨，分别计算。
- 新普通工程不内嵌峰值，便携 HTML 保留波形数据；缺少缓存不会阻止文字编辑和 TTS。
