...
from maw.openai_asr_capabilities import capabilities, validate_options
...
def normalize_base_url(value: str) -> str:
    ...
    if parsed.hostname and parsed.hostname.lower().rstrip('.') in {'openrouter.ai', 'www.openrouter.ai'} and parsed.path in {'', '/v1'}:
...
def _model_compatibility_hint(detail: str) -> str:
    ...
    if not any(
        ...
        return ""
    return (
        "提示：请检查模型名称是否与当前接口一致；使用中转站时，请选择“自定义（Custom）”，"
        "并填写服务商提供的完整模型名。"
    )
...
def request_transcription(
    ...
    *,
    ...
    diarize = validate_options(base_url, model_id, prompt or "", keywords or (), diarize)
    caps = capabilities(base_url, model_id)
    ...
    if not diarize and caps["granularities"]:
        data.extend([("timestamp_granularities[]", "segment"), ("timestamp_granularities[]", "word")])
    if language:
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_name:
        ...
        result = request_transcription(
            ...
            prompt=args.prompt, keywords=args.keyword, diarize=args.diarize,
        )
    ...
    print(f"全程总用时: {format_elapsed(elapsed)}")
    print(f"转写耗时: {format_elapsed(transcribe_elapsed)}")
    if duration > 0:
