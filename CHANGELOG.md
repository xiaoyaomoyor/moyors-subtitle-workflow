...
- **适配上游 beta.4 编辑器增量** ： 波形支持基于 RMS 响度自动定标，新工程默认自动，旧 MSW 手动振幅保持；保留源试听增益和既有共同边界联动。优化字幕列表在拆分、合并、撤销及保存中的阅读位置，新增跟随播放控制；颜色组支持单条脱离与中途改色。ASS 根据视频分辨率缩放字号，保留颜色及说话人，新增去空隙 ASS 导出。
